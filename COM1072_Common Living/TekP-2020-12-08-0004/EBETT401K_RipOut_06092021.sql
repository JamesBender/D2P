...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EBETT401K_20210609.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','BI1HN,H3M21,NZJU7,H3MBW',NULL,NULL,NULL,'Betterment 401K Export-Test','202105149','EMPEXPORT','TEST_XOE','May 21 2021 10:11AM','EBETT401K',NULL,NULL,NULL,'202105149','May 14 2021 12:00AM','Dec 30 1899 12:00AM','202105141','148','eecPayGroup','SALARY','202105141',dbo.fn_GetTimedKey(),NULL,'us3cPeCOM1072',NULL);
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvCheckDate =  CASE WHEN PrgPayDate is null THEN (SELECT top 1 prgPaydate from dbo.payreg with (nolock) where prgPaydate between @startdate and @enddate and prgpaygroup = 'SALARY') 
                                ...
                                END  --PrgPayDate
        ,drvBeginPayrollPeriod = CASE WHEN PrgPeriodStartDate is null THEN (SELECT top 1 PrgPeriodStartDate from dbo.payreg with (nolock) where prgPercontrol between @startpercontrol and @endpercontrol and prgpaygroup = 'SALARY') 
                                    ELSE dbo.dsi_fnGetMinMaxDates('MAX',, (SELECT top 1 PrgPeriodStartDate from dbo.payreg with (nolock) where prgPercontrol between @startpercontrol and @endpercontrol and prgpaygroup = 'SALARY') )  
                                END --PrgPeriodStartDate
        ,drvEndPayrollPeriod = CASE WHEN PrgPeriodEndDate is null THEN (SELECT top 1 PrgPeriodEndDate from dbo.payreg with (nolock) where prgPercontrol between @startpercontrol and @endpercontrol and prgpaygroup = 'SALARY') 
