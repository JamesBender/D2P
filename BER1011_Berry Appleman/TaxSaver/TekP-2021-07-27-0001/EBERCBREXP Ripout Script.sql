...
Client ID:      BER1011
Date/time:      2022-09-15 05:25:04.250
Ripout version: 7.4
Export Type:    Web
Status:         Production
Environment:    EWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'EBERCBREXP_20220915.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'Null','N',',IAGFG',NULL,NULL,NULL,'TaxSaver COBRA QB - Mon 6:10am','202005199','EMPEXPORT','SCH_TCBRX',NULL,'EBERCBREXP',NULL,NULL,NULL,'202209129','May  8 2020  3:58PM','May  8 2020  3:58PM','202209051',NULL,'','','202005191',dbo.fn_GetTimedKey(),NULL,'ULTI',NULL);
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','IAGFG',NULL,NULL,NULL,'Test Purposes Only','202208159','EMPEXPORT','TEST','Sep 14 2022  4:02PM','EBERCBREXP',NULL,NULL,NULL,'202208159','Aug  8 2022 12:00AM','Dec 30 1899 12:00AM','202207101','15','','','202207101',dbo.fn_GetTimedKey(),NULL,'us3lKiBER1011',NULL);
...
BEGIN
    ...
    DELETE FROM dbo.U_EBERCBREXP_EEList WHERE xEEID IN (
    ...
    SET @DedList = 'CGOAP,EPCGO,DNPPO,EPCDN,VIS,EPVSP,GLIFE,FSAMD,MEDC';
    ...
    INSERT INTO dbo.U_dsi_BDM_Configuration VALUES (@FormatCode,'RelationshipsChild','DAU,SON,DIS,CHL,DPC,STC,COD');
    ...
    INSERT INTO [dbo].[U_dsi_BDM_EBERCBREXP]
           ([BdmRecType]
           ,[BdmCOID]
           ,[BdmEEID]
           ,[BdmDepRecID]
    ...
    FROM dbo.emphded WITH (NOLOCK)
        JOIN dbo.U_dsi_BDM_DepDeductions 
            ON dbneeid = edheeid 
            AND dbnformatcode = 'EBERCBREXP'
    JOIN dbo.U_EBERCBREXP_Audit WITH (NOLOCK)
        ON audKey1Value = EdhEEID
    ...
    Delete from dbo.U_dsi_BDM_EBERCBREXP where bdmdedcode not in (Select dedcode from U_EBERCBREXP_DedList)
    ...
    SELECT DISTINCT
        ...
        ,drvEventType = CASE WHEN EecTermReason = '202' THEN 'RETIREMENT'
                             ...
                             END
        ,drvEventDate =  CASE 
                                  WHEN BdmCobraReason IN ('201','204','LEVNT4','LEVNT3') THEN (SELECT TOP 1 EdhEffDate FROM dbo.EmpHDed A WITH (NOLOCK) WHERE A.EdhEEID = xEEID AND A.EdhCoID = xCOID AND EdhChangeReason IN ('201','204','LEVNT4','LEVNT3') ORDER BY A.EdhEffDate DESC)
                                  --(select top 1 EjhJobEffDate from emphjob where ejheeid = xeeid and ejhcoid =  xcoid order by ejhdatetimecreated desc)
        ...
        ,drvMedicalCoverage = CASE WHEN (HDHP is not null)  THEN
                                        ...
                                        END
                              END
        ,drvDentalPlan = CASE WHEN DENHI is not null THEN (Select DedDesc from  dbo.U_EBERCBREXP_DedList where dedcode =  DENHI)
                              WHEN DENLO is not null THEN (Select DedDesc from  dbo.U_EBERCBREXP_DedList where dedcode =  DENLO)
                            ELSE 'NONE' END 
        ,drvDentalCoverage = CASE WHEN (DENHI is not null or DENLO is not null)  THEN
                                        ...
                                        END
                              END
        ,drvVisionPlan = CASE WHEN Vision is not null then (Select DedDesc from  dbo.U_EBERCBREXP_DedList where dedcode =  Vision) ELSE 'NONE' END
        ,Count_SPS
        ,Count_CHL
        ,Vision_BenOption
    ...
    JOIN (Select  A.bdmEEID,A.bdmcoid,
            ...
            ,HDHP = MAX((CASE WHEN BDMDedCode In ('CGOAP','EPCGO','MEDC') then BDMDedCode END))
            ...
            ,HDHP_benoption = MAX((CASE WHEN BDMDedCode In ('CGOAP','EPCGO','MEDC') then BdmBenOption END))
            ...
            ,Count(CASE WHEN BdmRelationship IN ('SPS') then 1 END) AS  Count_SPS
            --,Count(CASE WHEN BdmRelationship IN ('SPS') and BdmDedcode = 'V' then 1 END) AS  Count_SPS
            ,Count(CASE WHEN BdmRelationship IN ('DAU', 'SON', 'DIS', 'STC','CHL') then 1 END) AS  Count_CHL
 ...
 case when bdmdeprecid is null then eepGender else conGender end
    ...
    LEFT JOIN (Select bdmeeid AS KidEEID
        ...
        From (
                ...
                ON ConEEID = bdmeeid 
                AND ConSystemID = BdmDepRecID
                where bdmrelationship IN ('DAU', 'SON', 'DIS', 'STC','CHL') and bdmdedcode in ('CGOAP', 'EPCGO', 'DNPPO', 'EPCDN', 'VIS' , 'EPVSP', 'MEDC') 
        ...
        From (
                ...
                ON ConEEID = bdmeeid 
                AND ConSystemID = BdmDepRecID
                where bdmrelationship IN ('SPS','DP') and bdmdedcode in ('CGOAP', 'EPCGO', 'DNPPO', 'EPCDN', 'VIS' , 'EPVSP', 'MEDC')
                    group by bdmeeid, ConNameLast, ConNameFirst, ConSSN, ConDateOfBirth, ConRelationship
...
UPDATE dbo.AscExp
    SET expLastStartPerControl = '202207101'
       ,expStartPerControl     = '202207101'
