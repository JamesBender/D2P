...
/*08*/ DECLARE @FILENAME varchar(1000) = 'ECSB401KEX_20210309.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Test Purposes Only','202102261','EMPEXPORT','TEST','Mar  9 2021 12:00AM','ECSB401KEX',NULL,NULL,NULL,'202102261','Feb 26 2021 12:00AM','Dec 30 1899 12:00AM','202102261','6534','','','202102261',dbo.fn_GetTimedKey(),NULL,'us3mLaSUM1010',NULL);
...
CREATE TABLE [dbo].[U_ECSB401KEX_drvTbl] (
    ...
    [drvWorkHomePhone] varchar(50) NULL,
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,PehPayPerDefAllComp    = SUM(CASE WHEN ErnEarnCode IN ('BEREV','BMCRP','BMOVR','BMSAL','BONUS','COACH','COMM' ,'DSBNS','DT'   ,'FHOL' ,'FRIH' ,'FSBON','GRNT' ,'HOL'  ,'INDIS','JURY' ,'MUP'  ,'NWHR' ,'OT'   ,'OTHP' ,'OTHR' ,'PTO'  ,'PTOE' ,'REG'  ,'RETRG','RETRO','RTRBV','RTRDB','RTRFH','RTRFR','RTRLP','RTRNW','RTROT','RTRPE','RTRPT','SBONS','SFCCH','SICK' ,'SOBON','SPSL' ,'TRTFH','VAC'    ) AND PehPerControl >= @StartPerControl THEN PehCurAmt ELSE 0.00 END)
        ,PehPayPerTestingComp    = SUM(CASE WHEN ErnEarnCode IN ('BACP', 'INDPD', 'IDPM', 'IDPV', 'IMSC', 'NQDC', 'NQDC2', 'NQDC1', 'SEV'   ) AND PehPerControl >= @StartPerControl THEN PehCurAmt ELSE 0.00 END)
        ,PehPayPerTotalComp        = SUM(CASE WHEN ErnEarnCode IN ('BACP', 'INDPD', 'IDPM', 'IDPV', 'IMSC', 'NQDC', 'NQDC2', 'NQDC1', 'SEV'  ) AND PehPerControl >= @StartPerControl THEN PehCurAmt ELSE 0.00 END)
        ,PehYTDDefAllocComp        = SUM(CASE WHEN ErnEarnCode IN ('BEREV','BMCRP','BMOVR','BMSAL','BONUS','COACH','COMM' ,'DSBNS','DT'   ,'FHOL' ,'FRIH' ,'FSBON','GRNT' ,'HOL'  ,'INDIS','JURY' ,'MUP'  ,'NWHR' ,'OT'   ,'OTHP' ,'OTHR' ,'PTO'  ,'PTOE' ,'REG'  ,'RETRG','RETRO','RTRBV','RTRDB','RTRFH','RTRFR','RTRLP','RTRNW','RTROT','RTRPE','RTRPT','SBONS','SFCCH','SICK' ,'SOBON','SPSL' ,'TRTFH','VAC'  ) AND ErnInclInDefComp = 'Y' THEN PehCurAmt ELSE 0.00 END)
        ,PehYTDPlanTestComp        = SUM(CASE WHEN ErnEarnCode IN ('BACP', 'INDPD', 'IDPM', 'IDPV', 'IMSC', 'NQDC', 'NQDC2', 'NQDC1', 'SEV' ) THEN PehCurAmt ELSE 0.00 END)
        ,PehYTDPlanTotalComp    = SUM(CASE WHEN ErnEarnCode IN ('BACP', 'INDPD', 'IDPM', 'IDPV', 'IMSC', 'NQDC', 'NQDC2', 'NQDC1', 'SEV'  ) THEN PehCurAmt ELSE 0.00 END)
    ...
    SELECT DISTINCT
        ...
        ,drvContribSrc4 = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(PdhRothAmt*100, '#0'), ''), 12)
        ...
        ,drvPayPerDefAllComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PdhDedCalcBasisAmt <= 0  THEN PehPayPerDefAllComp*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
        ,drvPayPerTestingComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PdhDedCalcBasisAmt <= 0  THEN PehPayPerDefAllComp*100 ELSE PdhDedCalcBasisAmt*100 END*100, '#0'), ''), 12)
        ,drvPayPerTotalComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PdhDedCalcBasisAmt <= 0  THEN PehPayPerDefAllComp*100 ELSE PdhDedCalcBasisAmt*100 END*100, '#0'), ''), 12)
        ...
        ,drvPlanYTDConSrc4 = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(PdhRothAmtYTD*100, '#0'), ''),12)
        ...
        ,drvYTDDefAllocComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PdhDedCalcBasisAmtYTD <= 0  THEN PehYTDDefAllocComp*100 ELSE PdhDedCalcBasisAmtYTD*100 END, '#0'), ''), 12)
        
        ,drvYTDPlanTestComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PdhDedCalcBasisAmtYTD <= 0  THEN PehYTDPlanTestComp*100 ELSE PdhDedCalcBasisAmtYTD*100 END*100, '#0'), ''), 12)
        ,drvYTDPlanTotalComp =    RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PdhDedCalcBasisAmtYTD <= 0  THEN PehYTDPlanTotalComp*100 ELSE PdhDedCalcBasisAmtYTD*100 END*100, '#0'), ''), 12)
        ...
        ,drvWorkHomePhone = CASE WHEN EecPhoneBusinessNumber  is not null then EecPhoneBusinessNumber  else eepphonehomenumber END
    ...
    SET @Contrib4 = (SELECT SUM(CAST(drvContribSrc4Raw AS DECIMAL)) FROM dbo.U_ECSB401KEX_drvTbl);

    SET @Contrib5 = (SELECT SUM(CAST(drvContribSrc5Raw AS DECIMAL)) FROM dbo.U_ECSB401KEX_drvTbl);
    SET @Contrib6 = 0
    SET @Contrib7 = 0
    SET @Contrib8 = 0

    SET @Payment1 = (SELECT SUM(CAST(drvLoanPayment1Raw AS DECIMAL)) FROM dbo.U_ECSB401KEX_drvTbl);
    SET @Payment2 = 0
    SET @Payment3 = 0
    SET @Payment4 = 0
    SET @Payment5 = 0
