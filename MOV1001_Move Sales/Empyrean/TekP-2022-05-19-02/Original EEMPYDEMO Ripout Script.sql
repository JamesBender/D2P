...
Client ID:      MOV1001
Date/time:      2022-07-08 04:43:03.330
Ripout version: 7.4
Export Type:    Web
Status:         Production
Environment:    N33
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,NULL,NULL,',OG4IV,NR767',NULL,NULL,NULL,'Empyrean Demographic Sched','202108169','EMPEXPORT','SCHEDULED',NULL,'EEMPYDEMO',NULL,NULL,NULL,'202207069','Nov  1 2019  2:50PM','Nov  1 2019  2:50PM','202206291',NULL,'','','202108011',dbo.fn_GetTimedKey(),NULL,'ULTI',NULL);
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','OG4IV,NR767',NULL,NULL,NULL,'Empyrean Demographic Test','202203231','EMPEXPORT','TEST_XOE','Apr  5 2022  6:31PM','EEMPYDEMO',NULL,NULL,NULL,'202203231','Mar 23 2022 12:00AM','Dec 30 1899 12:00AM','202203231','1899','','','202203231',dbo.fn_GetTimedKey(),NULL,'US3CPEMOV1001',NULL);
    ...
    SELECT DISTINCT
       ...
       ,drvActionCode =    CASE WHEN TermActionCode IS NOT NULL THEN  TermActionCode
        ...
        ,drvActionReasonCode =    CASE WHEN TermActionCode IS NOT NULL THEN TermActionDesc
                                ...
                                END 
        -- field 62                                                     
        ,drvActionReasonCodeEffectiveDate =    CASE WHEN TermActionCode = 'PERSNL' OR ActActionCode = 'PERSNL' OR OejhReason = 'PERSNL' THEN   
                                                CASE WHEN EshStatusStopDate IS NOT NULL THEN EshStatusStopDate
                                                WHEN OejhReason IS NOT NULL THEN OEjhJobEffDate
                                                END
                                            WHEN TermActionCode IS NOT NULL THEN TermActionEffDate 
...
--               and JobLeave.ejhcoidleave = xcoid
    ...
    /*LEFT JOIN (
                SELECT A.EshEEID AS LpEEID, A.EshCOID AS LPCOID, A.EshEmplStatus AS LpStatus, A.EshStatusStartDate AS LpDate, ROW_NUMBER() OVER(PARTITION BY A.EshEEID, A.EshCOID ORDER BY A.EshStatusStartDate DESC) AS LpRN
                FROM EmpHStat A 
                JOIN (
