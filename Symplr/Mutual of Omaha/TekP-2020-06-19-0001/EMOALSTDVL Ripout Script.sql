...
UPDATE dbo.U_dsi_Configuration SET CfgValue = CASE WHEN CfgName = 'UseFileName' THEN 'Y' ELSE NULL END WHERE FormatCode = 'EMOALSTDVL' AND CfgName IN ('UseFileName','ExportPath');
INSERT INTO dbo.CustomTemplates (CreationDate,Engine,EngineCode,IsActive,ModifiedDate) SELECT CreationDate = GETDATE(), Engine = AdhEngine, EngineCode = AdhFormatCode, IsActive = 1, ModifiedDate = GETDATE() FROM dbo.AscDefH WITH (NOLOCK) WHERE AdhFormatCode = 'EMOALSTDVL' AND NOT EXISTS(SELECT 1 FROM dbo.CustomTemplates WHERE EngineCode = AdhFormatCode);
IF OBJECT_ID('U_EMOALSTDVL_SavePath') IS NOT NULL DROP TABLE [dbo].[U_EMOALSTDVL_SavePath];
GO
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvRelationshipCode =    CASE WHEN ConRelationship IN ('SPS','DP') AND ConGender = 'F' THEN 'W'
                                    ...
                                    WHEN ConRelationship IN ('CHD','CHL','DPC') AND ConGender = 'F' THEN 'D'
                                    WHEN ConRelationship IN ('CHD','CHL','DPC') AND ConGender = 'M' THEN 'S'
                                    ELSE 'M'
                                END
