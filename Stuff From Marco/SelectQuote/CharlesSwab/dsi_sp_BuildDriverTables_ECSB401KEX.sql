...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"SQI"','2','(''DA''=''F'')','ECSB401KEXZ0','3','D','10','4',NULL,'Plan ID',NULL,NULL);
...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"SQI"','2','(''DA''=''F'')','ECSB401KEXZ0','3','T','90','4',NULL,'Plan ID',NULL,NULL);
...
/*08*/ DECLARE @FILENAME varchar(1000) = 'ECSB401KEX_20210416.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Test Purposes Only','202103259','EMPEXPORT','TEST','Apr 16 2021  9:37AM','ECSB401KEX',NULL,NULL,NULL,'202103259','Mar 25 2021 12:00AM','Dec 30 1899 12:00AM','202103251','6591','','','202103251',dbo.fn_GetTimedKey(),NULL,'us3lKiSEL1005',NULL);
...
BEGIN
    ...
    SET @DedList = '700,701,703,704,MATCH,765,766,767,768,769,771,775,PROSH';
    ...
    SELECT DISTINCT
        ...
        ,Pdh401FAmtYTD    = SUM(CASE WHEN PdhDedCode IN ('700','701')  AND PdhPerControl >=  CAST( (YEAR(@StartDate) - 1) as varchar) + '07011'  THEN PdhEECurAmt ELSE 0.00 END)
        ...
        ,Pdh401CAmtYTD    = SUM(CASE WHEN PdhDedCode IN ('401C')   AND PdhPerControl >=  CAST( (YEAR(@StartDate) - 1) as varchar) + '07011' THEN PdhEECurAmt ELSE 0.00 END)
        ...
        ,PdhRothAmtYTD    = SUM(CASE WHEN PdhDedCode IN ('703','704')   AND PdhPerControl >=  CAST( (YEAR(@StartDate) - 1) as varchar) + '07011' THEN PdhEECurAmt ELSE 0.00 END)        
        ...
        ,PdhMatchAmtYTD    = SUM(CASE WHEN PdhDedCode IN ('MATCH')  AND PdhPerControl >=  CAST( (YEAR(@StartDate) - 1) as varchar) + '07011' THEN PdhERCurAmt ELSE 0.00 END)
        ,Pdh401L1Amt     = SUM(CASE WHEN PdhDedCode IN ('765', '766', '767', '768', '769', '771', '775') AND PdhPerControl BETWEEN @StartPerControl AND @EndPerControl THEN PdhEECurAmt ELSE 0.00 END)
        ,PdhDedCalcBasisAmt =  SUM(CASE WHEN PdhDedCode IN ('700','701','703','704','MATCH') AND PdhPerControl > CAST( (YEAR(@StartDate) - 1 ) as varchar) + '06011' THEN PdhEECurAmt ELSE 0.00 END)
    ...
    SELECT DISTINCT
        ...
        ,drvPayPerDefAllComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PehInclInDefComp > 0  THEN PehInclInDefComp*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
        ,drvPayPerTestingComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PehInclInDefComp > 0  THEN PehInclInDefComp*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
        ,drvPayPerTotalComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PehInclInDefComp > 0  THEN PehInclInDefComp*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
        ...
        ,drvYTDDefAllocComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PehInclInDefCompYTD > 0  THEN PehInclInDefCompYTD*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
        ,drvYTDPlanTestComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PehInclInDefCompYTD > 0  THEN PehInclInDefCompYTD*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
        ,drvYTDPlanTotalComp = RIGHT(REPLICATE('0', 12) + ISNULL(FORMAT(CASE WHEN PehInclInDefCompYTD > 0  THEN PehInclInDefCompYTD*100 ELSE PdhDedCalcBasisAmt*100 END, '#0'), ''), 12)
