...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EDISCCOBEX_20210408.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Discovery Cobra Combined-Test','202104089','EMPEXPORT','TEST_XOE','Apr  8 2021  1:56PM','EDISCCOBEX',NULL,NULL,NULL,'202104089','Apr  8 2021 12:00AM','Dec 30 1899 12:00AM','202101011','332','','','202101011',dbo.fn_GetTimedKey(),NULL,'us3lKiNET1005',NULL);
...
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EDISCCOBEX','SubSort','C','drvSubSort');
...
BEGIN
    ...
    SET @DedList = 'DENT,EYE,FSAL,FSA,MEDG,MEDP,MEDS,MED3';
...
INSERT INTO [dbo].[U_dsi_BDM_EDISCCOBEX]
 ...
 Select rectype = 'DEP', EdhCoid, EdhEEID, DbnDepRecID, DbnDepRecID, 'QB', 'Data Inserted for 201 Chg reason', 
            ...
            and dbnrelationship = 'CHL' and dbnrunid  = 'QB'
    ...
    SELECT DISTINCT
        ...
        ,drvEventType =    CASE WHEN BdmCobraReason IN ('204','LEVNT4') THEN 'DIVORCELEGALSEPARATION'
                        ...
                        END
        ,drvEventDate = BdmDateOFCobraEvent /*   CASE WHEN BdmCobraReason IN ('204','LEVNT4','201','302','LEVNT3') THEN EdhEffDate
                                --CASE WHEN ISNULL(ConCobraStatusDate, '') <> '' THEN ConCobraStatusDate ELSE BdmDateOFCobraEvent END
                            ELSE BdmDateOFCobraEvent 
                        END*/
        ,drvEnrollmentDate = BdmBenStartDate --CASE WHEN BdmCobraReason IN ('DENT','EYE','FSAL','FSA','MEDG','MEDP','MEDS','MED3') THEN EdhEffDate ELSE BdmBenStartDate END
        ,drvSSN = eepSSN
   ...
   JOIN (SELECT
          ...
          FROM dbo.U_dsi_BDM_EDISCCOBEX WITH (NOLOCK)
          WHERE BdmRunID = 'QB'
            AND BdmIsPQB = 'Y'
    ...
    SELECT DISTINCT
        ...
        ,drvDepRecID = CONVERT(varchar(12),'1') --DELETE IF NOT USING DEPENDENT DATA
        ,drvSort = '3 ' + xEEID + ' 3'
        ,drvSubSort =  CASE WHEN BdmDedCode = 'MEDG' THEN '1'
                            WHEN BdmDedCode in ('MEDP','MED3') THEN '2'
                            WHEN BdmDedCode = 'MEDS' THEN '3'
                            WHEN BdmDedCode = 'DENT' THEN '4'
                            WHEN BdmDedCode IN ('FSA','FSAL') THEN '6'
                            WHEN BdmDedCode = 'EYE' THEN '5'
                        END
        -- standard fields above and additional driver fields below
        ,drvPlanName =    CASE WHEN BdmDedCode = 'MEDG' THEN 'Centivo Medical Gold'
                            WHEN BdmDedCode in ('MEDP','MED3') THEN 'Centivo Medical Platinum'
        ...
        ,drvCoverageLevel =  CASE 
                                ...
                                WHEN BdmDedCode IN ('MEDG','MEDP','MEDS','DENT','EYE','MED3') THEN 
    ...
    JOIN (
            ...
            FROM (
                    ...
                    WHERE EdhDedCode IN ('DENT','EYE','FSAL','FSA','MEDG','MEDP','MEDS','MED3') 
