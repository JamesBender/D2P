...
Client ID:      PRI1029
Date/time:      2022-01-27 15:47:35.670
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    NWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'EBOAHSA_20220127.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','',NULL,NULL,NULL,'Test Purposes Only','202201289','EMPEXPORT','TEST','Jan 27 2022  3:00PM','EBOAHSA',NULL,NULL,NULL,'202201289','Jan 28 2022 12:00AM','Dec 30 1899 12:00AM','202201281','2138','eecPayGroup','P1,HOURLY','202201281',dbo.fn_GetTimedKey(),NULL,'us3cPePRI1029',NULL);
...
EXEC dbo._dsi_usp_ExportRipOut_v7_4 @FormatCode = 'EBOAHSA', @AllObjects = 'Y', @IsWeb = 'Y'
**********************************************************************************/
BEGIN
    ...
    SELECT 
        ...
        ,drvPlanName = CASE 
                       ...
                       --WHEN EedDedCode IN ('HSA','HSAI','HSAF','HSI') THEN 'Health Savings Account' 
                       END
        ,drvContribDate = (SELECT MAX(DATEADD(day,-2, PrgPayDate)) FROM dbo.vw_int_PayReg WITH (NOLOCK) WHERE PrgEEID = xEEID AND PrgCOID = xCOID AND PrgPayDate BETWEEN @StartDate AND @EndDate)
        ,drvContribDesc = CASE 
...
UPDATE dbo.AscExp
    SET expLastStartPerControl = '202112172'
       ,expStartPerControl     = '202112172'
       ,expLastEndPerControl   = '202112172'
       ,expEndPerControl       = '202112172'
       ,expSelectByField       = 'eecPayGroup'
       ,expSelectByList        = 'P1'
WHERE expFormatCode = 'EBOAHSA' AND expExportCode like '%TEST%';
**********************************************************************************/
GO
