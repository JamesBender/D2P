...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EHFXCENTE2_20210419.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'TEST','202104169','EMPEXPORT','TEST','Apr 16 2021 10:53AM','EHFXCENTE2',NULL,NULL,NULL,'202104169','Apr 16 2021 12:00AM','Dec 30 1899 12:00AM','202104021','959','','','202104021',dbo.fn_GetTimedKey(),NULL,'us3lKiTEN1000',NULL);
...
--        drvPeriodEndDate = CONVERT(varchar(10), PgpPeriodEndDate, 112)--case when eecemplstatus <> 'T' then CONVERT(varchar(10), PgpPeriodEndDate, 112) end
    ,drvStatChgDate = CONVERT(VARCHAR(10), yEjhJobEffDate, 112)
  ...
  from dbo.u_EHFXCENTE2_eelist with (nolock)
    ...
    LEFT JOIN (
                ...
                JOIN (
                        ...
                        FROM (
                                SELECT EjhEEID, EjhCOID, EjhFullTimeOrPartTime, EjhJobEffDate, ROW_NUMBER() OVER (PARTITION BY EjhEEID, EjhCOID, EjhFullTimeOrPartTime ORDER BY EjhJobEffDate DESC) AS RN
