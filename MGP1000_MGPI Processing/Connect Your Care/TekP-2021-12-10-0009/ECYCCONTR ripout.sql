...
Client ID:      MGP1000
Date/time:      2022-02-19 09:13:56.507
Ripout version: 7.4
Export Type:    Web
Status:         Production
Environment:    EWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'ECYCCONTR_20220219.txt';
...
EXEC dbo._dsi_usp_ExportRipOut_v7_4 @FormatCode = 'ECYCCONTR', @AllObjects = 'Y', @IsWeb = 'Y'
**********************************************************************************/
BEGIN
...
WHERE adfHeaderSystemID = (Select AdhSystemID FROM dbo.AscDefH WITH (NOLOCK) WHERE AdhFormatCode='ECYCCONTR')
