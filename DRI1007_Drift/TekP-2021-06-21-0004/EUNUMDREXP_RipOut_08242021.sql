...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EUNUMDREXP_20210824.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'Null','N',',LH765',NULL,NULL,NULL,'UNUM FMLA LTD STD-Fri 8am','202107269','EMPEXPORT','SCH_EUNUMD',NULL,'EUNUMDREXP',NULL,NULL,NULL,'202108209','Jul 26 2021  4:24PM','Jul 26 2021  4:24PM','202108131',NULL,'','','202107261',dbo.fn_GetTimedKey(),NULL,'ULTI',NULL);
...
08/24/2021 by AP:
		- Updated Date Of Last Salary change to compare between last hire date and last job history date.
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvRecentHireDt = Eec.EecDateOfLastHire
        ...
        ,drvSickPayHrs = '0'
        ,drvDtLastSalChg =  CASE WHEN EecM.EecDateOfLastHire > Ejh.EjhJobEffDate THEN EecM.EecDateOfLastHire ELSE Ejh.EjhJobEffDate END
		--CASE WHEN Ejh.EjhIsRateChange = 'N' THEN EecDateOfLastHire ELSE dbo.dsi_fnlib_GetAnnSalary_EffDate_WithStartDate(EecEEID, EecCOID, CAST(GETDATE() AS DATE),EecDateOfLastHire) END
        ...
        ,drvEarnings1 = CASE WHEN YEAR(Eec.EecDateOfLastHire) < YEAR(GETDATE()) THEN  
                                CASE WHEN STD IS NOT NULL THEN CAST(CONVERT(DECIMAL(10,2), Peh.PehCurAmtYTD / 52) AS VARCHAR) END 
                            WHEN YEAR(Eec.EecDateOfLastHire) = YEAR(GETDATE()) THEN
        ...
        ,drvEarnings2 = CASE WHEN YEAR(Eec.EecDateOfLastHire) < YEAR(GETDATE()) THEN  
        ...
        ,drvEarnings3 = CASE WHEN YEAR(Eec.EecDateOfLastHire) < YEAR(GETDATE()) THEN  
	...
	LEFT JOIN (SELECT EecEEID, EecCOID, MAX(EecDateOfLastHire) EecDateOfLastHire
				FROM dbo.EmpComp WITH(NOLOCK)
				GROUP BY EecEEID, EecCOID) EecM ON EecM.EecEEID = xEEID AND EecM.EecCOID = xCOID
