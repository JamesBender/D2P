...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvGrade = SGSalaryGrade
        ,drvPeopleManager = dbo.dsi_fnlib_IsSupervisor(xEEID)
        ,drvUnion = CASE WHEN SGUnionLocal in ('TEAM', 'UFW') THEN 'Union' ELSE 'Non-Union' END
	...
	LEFT JOIN (SELECT EJHEEID AS SGEEID, EjhSalaryGrade AS SGSalaryGrade, EJHUnionLocal AS SGUnionLocal, MAX(EjhJobEffDate) AS SGJobEffDate
				FROM dbo.EmpHJob WITH (NOLOCK)
				GROUP BY EJHEEID, EjhSalaryGrade,EJHUnionLocal) AS SG
		ON SGEEID = xEEID 
