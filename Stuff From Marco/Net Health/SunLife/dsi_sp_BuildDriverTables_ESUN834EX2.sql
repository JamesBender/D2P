...
/*08*/ DECLARE @FILENAME varchar(1000) = 'ESUN834EX2_20210310.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Full File Only','202103039','EMPEXPORT','FULLFILE','Mar  3 2021  6:32PM','ESUN834EX2',NULL,NULL,NULL,'202103039','Mar  3 2021 12:00AM','Dec 30 1899 12:00AM','202102171','25133','','','202102171',dbo.fn_GetTimedKey(),NULL,'us3lKiNET1005',NULL);
...
BEGIN
    ...
    SET @DedList = 'GLIFO,GLIFE,LTDN,LTDB,EAP,STD,LIFEE,LIFES,LIFEC';
    ...
    ***************************************************************************************************************/
     ...
     SELECT Bdmeeid,
            ...
            CASE    WHEN BdmDedCode IN ('GLIFE','GLIFO') THEN 'AC'
                    ...
                    WHEN BdmDedCode IN ('LIFEE', 'LIFES', 'LIFEC') THEN 'AF'                              
                END as code, 
  ...
  -------------------
    ...
    ***************************************************************************************************************/
    ...
    SELECT DISTINCT
        ...
        */
        ,drvAMT00_AmountQualifierCode1 = CASE WHEN LIFEE is not null or LIFES  is not null or LIFEC is not null or LTDN is not null THEN 'AMT' END
        ...
        ,drvAMT02_MonetaryAmount1 =    CASE 
                                            ...
                                            WHEN LTDN is not null  THEN  (Select Max(LTDN) from U_ESUN834EX2_BdmConsolidated A where A.bdmeeid = xeeid and A.bdmcoid = xcoid)
                                    END
    ...
    JOIN dbo.Contacts
    ON Coneeid = drveeid
    AND ConRelationship IN ('CHL','STC') and ConIsDependent = 'Y' and CONVERT(CHAR(9),RTRIM(drvSSN)) + CONVERT(CHAR(12),ISNULL(ConSystemID,''))
                      + CASE WHEN drvHD03_InsuranceLineCode IN ('AF') THEN '15'
