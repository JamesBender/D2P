...
/*08*/ DECLARE @FILENAME varchar(1000) = 'ETRUSTKWPM_20210729.txt';
...
;
  ...
  --                  WHEN t.is_crile = '1' and t.is_crill is null and t.is_crils is null then 'Employee' END
		...
		LEFT JOIN (SELECT ConEEID, '15000' AS Spouse_Ben_Amt
					...
					WHERE ConRelationship IN ('SPS', 'DP')) sp ON sp.ConEEID = Con.ConEEID
		LEFT JOIN (SELECT ConEEID, '3000' AS Child_Ben_Amt
					FROM dbo.Contacts WITH(NOLOCK)
					WHERE ConRelationship IN ('CHL', 'DCH', 'DPC', 'STC')) chl ON chl.ConEEID = Con.ConEEID
        WHERE EecEEType <> 'TES'
