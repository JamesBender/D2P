...
Client ID:      INT1073
Date/time:      2023-10-09 12:39:18.793
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    EWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'EABGFSADXP_20231009.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'Null','N',',BIDWP',NULL,NULL,NULL,'ABG FSA Export-Sched','202110299','EMPEXPORT','SCH_EABGFS','Oct 29 2021 11:45AM','EABGFSADXP',NULL,NULL,NULL,'202310049','Oct 29 2021 10:08AM','Oct 29 2021 10:08AM','202309271','125','','','202110291',dbo.fn_GetTimedKey(),NULL,'ULTI',NULL);
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','',NULL,NULL,NULL,'ABG FSA Export-Test','202310041','EMPEXPORT','TEST_XOE','Oct  9 2023 12:33PM','EABGFSADXP',NULL,NULL,NULL,'202310041','Oct  4 2023 12:00AM','Dec 30 1899 12:00AM','202309271','141','','','202309271',dbo.fn_GetTimedKey(),NULL,'us3mWaINT1073',NULL);
...
Marie Waters        10/9/2023                               Updated plan year for no OE to use @EndDate vs Startdate due to overlap
...
BEGIN
  ...
  --      ,drvERPlanYearEndDate =    CASE WHEN @ExportCode LIKE 'OE%' THEN  CAST(cast (DATEPART(YEAR,DATEADD(YEAR, 1, @StartDate)) as varchar ) + '0930' AS DATE)  -- MW added 3/23/23
        --                             WHEN @ExportCode NOT LIKE 'OE%' and DATEPART(MONTH, @StartDate) > 9  THEN CAST(cast( DATEPART(YEAR, DATEADD(YEAR, 1, @StartDate)) as varchar ) + '0930' AS DATE) ELSE CAST(cast( DATEPART(YEAR, @StartDate)  as varchar ) + '0930' AS DATE)END
         ...
         ,drvERPlanYearStartDate = CASE WHEN @ExportCode LIKE 'OE%'  THEN  CAST(cast( DATEPART(YEAR,@StartDate) as varchar ) + '1001' AS DATE)  -- MW added 3/23/23
                                       WHEN @ExportCode NOT LIKE 'OE%' and DATEPART(MONTH, @ENDDATE) < 10 THEN CAST(cast( DATEPART(YEAR,DATEADD(YEAR, -1, @ENDDATE)) as varchar ) + '1001' AS DATE) ELSE CAST(cast( DATEPART(YEAR, @ENDDATE)  as varchar ) + '1001' AS DATE) 
                                      END
        ---drvERPlanYearStateDate = CAST('10/1/' + FORMAT(CASE WHEN DATEPART(MONTH, @StartDate) < 10 THEN DATEPART(YEAR, DATEADD(YEAR, -1, @StartDate)) ELSE  DATEPART(YEAR, @StartDate) END, '0000') AS DATE) -- MW changed to above 3/23/23
        ,drvERPlanYearEndDate =    CASE WHEN @ExportCode LIKE 'OE%' THEN  CAST(cast (DATEPART(YEAR,DATEADD(YEAR, 1, @StartDate)) as varchar ) + '0930' AS DATE)  -- MW added 3/23/23
                                     WHEN @ExportCode NOT LIKE 'OE%' and DATEPART(MONTH, @ENDDATE) > 9  THEN CAST(cast( DATEPART(YEAR, DATEADD(YEAR, 1, @ENDDATE)) as varchar ) + '0930' AS DATE) ELSE CAST(cast( DATEPART(YEAR, @ENDDATE)  as varchar ) + '0930' AS DATE)END
