...
GO
IF OBJECT_ID('U_EUNUBDRIFT_count') IS NOT NULL DROP TABLE [dbo].[U_EUNUBDRIFT_count];
GO
...
INSERT INTO [dbo].[AscDefH] (AdhAccrCodesUsed,AdhAggregateAtLevel,AdhAuditStaticFields,AdhChildTable,AdhClientTableList,AdhCustomDLLFileName,AdhDedCodesUsed,AdhDelimiter,AdhEarnCodesUsed,AdhEEIdentifier,AdhEndOfRecord,AdhEngine,AdhFileFormat,AdhFormatCode,AdhFormatName,AdhFundCodesUsed,AdhImportExport,AdhInputFormName,AdhIsAuditFormat,AdhIsSQLExport,AdhModifyStamp,AdhOutputMediaType,AdhPreProcessSQL,AdhRecordSize,AdhSortBy,AdhSysFormat,AdhSystemID,AdhTaxCodesUsed,AdhYearStartFixedDate,AdhYearStartOption,AdhRespectZeroPayRate,AdhCreateTClockBatches,AdhThirdPartyPay) VALUES ('N','C','Y','0','','','N','','N','','013010','EMPEXPORT','CDE','EUNUBDRIFT','Unum Basic LTD STD Export','N','E','FORM_EMPEXPORT','N','C',dbo.fn_GetTimedKey(),'D','dbo.dsi_sp_Switchbox_v2','2000','S','N','EUNUBDRIFTZ0','N','Jan  1 1900 12:00AM','C','N',NULL,'N');
...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EUNUBDRIFT_20210831.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Unum Basic LTD STD Export','202108099','EMPEXPORT','FULLFILE','Aug  9 2021  9:51AM','EUNUBDRIFT',NULL,NULL,NULL,'202108099','Aug  9 2021 12:00AM','Dec 30 1899 12:00AM','202107261','1172','','','202107261',dbo.fn_GetTimedKey(),NULL,'us3lKiDRI1007',NULL);
...
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EUNUBDRIFT','ExportPath','V',NULL);
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EUNUBDRIFT','InitialSort','C','drvInitialSort');
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EUNUBDRIFT','SubSort','C','drvSubSort');
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EUNUBDRIFT','Testing','V','Y');
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EUNUBDRIFT','UseFileName','V','Y');
...
IF OBJECT_ID('U_EUNUBDRIFT_count') IS NULL
CREATE TABLE [dbo].[U_EUNUBDRIFT_count] (
    [c] varchar(30) NULL
);
...
----------------
08/06/2021 by AP:
        - Added 2nd insert statement for MA employees.
        - Updated term reason coding.
        - Updated effective date.

08/11/2021 by AP:
        - Fixed duplicate entries due to incorrect usage of ded codes GTL more than once in drvEffDt.
        - Removed the EedDedCode section from drvEffDt.
        - Added UPDATE statement to handle the 1 employee who has NULL for drvEffDt.
        - Fixed amount to use the YTD amount for Peh which includes the 12 month interval timeframe.

08/24/2021 by AP:
        - Updated logic for eEnroll2 to only show on 1 line.
        - Updated effective date to have a minimum date of 07/01/21.
        - Updated gender logic.

08/31/2021 by AP:
		- Changed YTD logic for Peh to be between @EndPerControl instead of between -12 months and @StartPerControl.
...
BEGIN
    ...
    SELECT DISTINCT
     ...
     --   ,PehEarnCode
        ...
        ,PehCurAmtYTD           = SUM(CASE WHEN (PehPerControl >= @Last12MonthsPerControl AND PehPerControl <= @EndPerControl) AND PehEarnCode IN ('REG', 'REGE', 'PTO', 'HOL', 'RETRO', 'SICK', 'COV19') THEN PehCurAmt ELSE 0.00 END)
		--SUM(CASE WHEN (PehPerControl >= @Last12MonthsPerControl AND PehPerControl <= @StartPerControl) AND PehEarnCode IN ('REG', 'REGE', 'PTO', 'HOL', 'RETRO', 'SICK', 'COV19') THEN PehCurAmt ELSE 0.00 END)
    ...
    SELECT DISTINCT
        ...
        ,drvEnroll2 = CONVERT(CHAR(6), '') -- UPDATE LATER
        ,drvPolicy = '449511'
        --CASE WHEN EepAddressState = 'MA' THEN '449512'
                           -- ELSE '449511' END
        ,drvDivision = '0001'
        ,drvMemId = EepSSN
        ,drvDOH = EecDateOfLastHire
        ,drvSalary = CAST(CAST(PehCurAmtYTD AS DECIMAL(10,2)) AS VARCHAR)
        --CAST(CAST(PehCurAmt AS DECIMAL(10,2)) AS VARCHAR)
        ...
        ,drvGender = CASE WHEN EepGender NOT IN ('M', 'F') THEN 'F' ELSE EepGender END
        ,drvDOB = EepDateOfBirth
        ,drvEmail = EepAddressEMail
        ,drvClass = '1101'
        --CASE WHEN EepAddressState = 'MA' THEN '0100'
                        --ELSE '1101' END
        ,drvEffDt = CASE WHEN audReHire = 'Y' THEN EecDateOfLastHire
                        ...
                        ELSE dbo.dsi_fnGetMinMaxDates('MAX', EedBenStartDate, @FileMinCovDate) END
                      --  WHEN EedDedCode IN ('LTD', 'STD', 'GLIFE') THEN EedBenStartDate
    ...
    LEFT JOIN (SELECT EedEEID, EedCOID, MAX(EedBenStartDate) EedBenStartDate, MAX(EedBenStopDate) EedBenStopDate
            ...
            AND EedDedCode IN ('STD', 'LTD', 'GLIFE')
    ...
    LEFT JOIN (
        ...
        GROUP BY audEEID
         ...
         ON audEEID = xEEID
    WHERE EecEEType = 'REG'
    --AND EepAddressState <> 'MA'

    INSERT INTO dbo.U_EUNUBDRIFT_drvTbl
     SELECT DISTINCT
         drvEEID = xEEID
        ,drvCoID = xCoID
        ,drvDepRecID = CONVERT(varchar(12),'1') --DELETE IF NOT USING DEPENDENT DATA
  ...
  --  INTO dbo.U_EUNUBDRIFT_drvTbl
    ...
    -- Update U_EUNUBDRIFT_drvTbl to update drveffdates that are NULL
    --UPDATE dbo.U_EUNUBDRIFT_drvTbl
    --SET drvEffDt = (SELECT TOP 1 CASE drvEffDt 
    --                        WHEN NULL THEN EecDateOfLastHire ELSE drvEffDt END
    --                        FROM dbo.EmpComp WITH(NOLOCK)
    --                        WHERE dbo.U_EUNUBDRIFT_drvTbl.drvEEID = EecEEID
    --                        AND dbo.U_EUNUBDRIFT_drvTbl.drvCOID = EecCOID)

    -- Update drvEnroll2 field with record count

    --Update Header information for Enroll2 fields

    Update TOP (1)  U_EUNUBDRIFT_drvTbl set drvEnroll2 = (SELECT CONVERT(CHAR(6),count(*)) from U_EUNUBDRIFT_drvTbl) FROM  U_EUNUBDRIFT_drvTbl
    where drvInitialSort  = (select top (1) drvInitialSort from dbo.U_EUNUBDRIFT_drvTbl order by drvInitialSort)
