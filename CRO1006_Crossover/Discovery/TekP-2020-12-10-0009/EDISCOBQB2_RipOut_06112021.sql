...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EDISCOBQB2_20210611.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','SPXBO,SPXK9',NULL,NULL,NULL,'Test Purposes Only','202105289','EMPEXPORT','TEST','May 28 2021  3:27PM','EDISCOBQB2',NULL,NULL,NULL,'202105289','May 28 2021 12:00AM','Dec 30 1899 12:00AM','202104011','232','','','202104011',dbo.fn_GetTimedKey(),NULL,'us3rVaCRO1006',NULL);
...
BEGIN
	...
	-- Create table to re-create coverage logic
		...
		SELECT e.bdmeeid, e.bdmcoid, count(distinct(s.bdmdateofbirth)) as spouse_count, count(distinct(c.bdmdateofbirth)) as child_count
		into dbo.U_EDISCOBQB2_coverage 
		FROM dbo.U_dsi_BDM_EDISCOBQB2 e WITH(NOLOCK) 
		left JOIN dbo.U_dsi_BDM_EDISCOBQB2 s WITH(NOLOCK) ON s.bdmeeid = e.bdmeeid  and s.bdmrelationship in ('sps', 'dp', 'dpn')
		left join dbo.U_dsi_BDM_EDISCOBQB2 c with(nolock) on c.bdmeeid = e.bdmeeid and c.bdmrelationship in ('chl', 'stc', 'dpc', 'ddp', 'grc')
    ...
    LEFT JOIN dbo.Company WITH (NOLOCK)
        ON CmpCOID = BdmCOID
    WHERE BdmIsPQB = 'Y'
        AND BdmDedCode IN  ('XOMFT','XOMPT','DENFT','DENPT','VISFT','VISPT','MEDFT','MEDPT','FSA','FSAPT','LFAPT','LPFFT') --('XOMFT','XOMPT')
    ;
    ---------------------------------
    ...
    SELECT DISTINCT
        ...
        ,drvEventType = CASE 
                                    ...
                                    ELSE 'TERMINATION'
                        END
        ,drvEventDate = DATEADD(DAY, 1, BdmDateOfCOBRAEvent) -- Day of COBRA EVENT + 1
    ...
    JOIN (SELECT
              ...
              FROM dbo.U_dsi_BDM_EDISCOBQB2 WITH (NOLOCK)
              WHERE BdmIsPQB = 'Y' AND BdmDedCode IN ('XOMFT','XOMPT','DENFT','DENPT','VISFT','VISPT','MEDFT','MEDPT','FSA','FSAPT','LFAPT','LPFFT') -- ('XOMFT','XOMPT')
    ...
    SELECT DISTINCT
        ...
        ,drvPlanName =    CASE WHEN A.BdmDedCode IN ('XOMFT','XOMPT') THEN 'Aetna Medical XO First PPO' -- jcb
                        ...
                        END                    
        ,drvStartDate = DATEADD(DAY, 1, A.BdmBenStopDate)
        ,drvCoverageLevel =    CASE WHEN A.BdmCobraReason IN ('200','201','202','204','205','206','207','208','209','210','LEVNT1','LEVNT2','LEVNT3','LEVNT4','LEVNT5') THEN
								  CASE WHEN c.spouse_count >= 1 and c.child_count = 0 THEN 'EE+SPOUSE'
                                ...
                                --        END
                                    ...
                                    ELSE --'EE'
                                        CASE WHEN (SELECT COUNT(*) FROM dbo.U_dsi_BDM_EDISCOBQB2 B WITH (NOLOCK) WHERE A.BdmEEID = B.BdmEEID AND B.BdmDedCode IN ('XOMFT','XOMPT') AND B.BdmRelationship IN ('CHL','DPC','STC','DCH')) = 1 THEN 'EE+CHILD'
                                            ...
                                            ELSE 'EE'
                                        END*/
        ,drvPlanBundle = CASE WHEN BdmDedCode IN ('XOMFT', 'XOMPT') THEN 'Aetna Medical XO First PPO/Discovery Benefits HRA' ELSE '' END
        ,drvSubSort =    EepSSN /*CASE WHEN BdmRecType = 'EMP' THEN eepSSN
                            ELSE ISNULL(NULLIF(ConSSN,''),eepSSN) + CONVERT(char(8),ConDateOfBirth,112)
                        END*/
        ,drvSubSort2 = '2'
    ...
    WHERE BdmDedCode IN ('XOMFT','XOMPT','DENFT','DENPT','VISFT','VISPT','MEDFT','MEDPT','FSA','FSAPT','LFAPT','LPFFT')
        AND BdmIsPQB = 'Y'
    ;
    ...
    SELECT DISTINCT
        ...
        ,drvDepRecID = '' --drvDepRecID
        ,drvRecordIdentifier = '[QBPLAN]'
        ,drvPlanName = CASE WHEN BdmDedCode IN ('XOMFT','XOMPT') THEN 'Discovery Benefits HRA' END                    
    ...
    JOIN dbo.U_dsi_BDM_EDISCOBQB2 WITH (NOLOCK)
     ...
     JOIN dbo.Contacts WITH (NOLOCK)
        ...
        AND ConSystemID = BdmDepRecID
    WHERE --BdmIsPQB = 'N'
         BdmDedCode IN ('XOMFT','XOMPT','DENFT','DENPT','VISFT','VISPT','MEDFT','MEDPT','FSA','FSAPT','LFAPT','LPFFT') --('XOMFT','XOMPT')
    ...
    --AND BdmDedCode IN ('XOMFT','XOMPT')
         BdmDedCode IN  ('XOMFT','XOMPT','DENFT','DENPT','VISFT','VISPT','MEDFT','MEDPT','FSA','FSAPT','LFAPT','LPFFT')
    ...
    SELECT DISTINCT
		...
		,drvStartDate = DATEADD(DAY, 1, BdmDateOfCOBRAEvent)
