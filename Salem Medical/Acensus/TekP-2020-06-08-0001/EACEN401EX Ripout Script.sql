...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,PdhEmployee401k     = SUM(CASE WHEN PdhDedCode IN ('401K', '401CU') THEN PdhEECurAmt ELSE 0.00 END)
        ,PdhRoth401k     = SUM(CASE WHEN PdhDedCode IN ('ROTH') THEN PdhEECurAmt ELSE 0.00 END)
        ,PdhLoanPaymentAmt     = SUM(CASE WHEN PdhDedCode IN ('401LN') THEN PdhERCurAmt ELSE 0.00 END)        
