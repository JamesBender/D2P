...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EUNUBDRIFT_20210806.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Unum Basic LTD STD Export','202108049','EMPEXPORT','FULLFILE','Aug  4 2021 10:35AM','EUNUBDRIFT',NULL,NULL,NULL,'202108049','Aug  4 2021 12:00AM','Dec 30 1899 12:00AM','202107211','488','','','202107211',dbo.fn_GetTimedKey(),NULL,'us3lKiDRI1007',NULL);
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvEffDt = CASE WHEN audReHire = 'Y' THEN EecDateOfLastHire
                        ...
                        WHEN EedDedCode IN ('GLIFE', 'STD', 'GLIFE') THEN EedBenStartDate
    ...
    JOIN dbo.U_EUNUBDRIFT_PEarHist WITH(NOLOCK)
        ON PehEEID = xEEID
        AND PehCOID = xCOID
    LEFT JOIN (SELECT EedEEID, EedCOID, EedDedCode, MAX(EedBenStartDate) EedBenStartDate, MAX(EedBenStopDate) EedBenStopDate
            FROM dbo.u_dsi_bdm_EmpDeductions WITH(NOLOCK)
            WHERE EedFormatCode = @FormatCode
            AND EedValidForExport = 'Y'
            GROUP BY EedEEID, EedCOID, EedDedCode) Eed ON Eed.EedEEID = xEEID AND Eed.EedCOID = xCOID
