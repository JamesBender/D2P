...
Client ID:      MEN1004
Date/time:      2022-02-04 21:27:20.357
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    EWP
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','',',1ZGJJ,1ZGD2,1ZGB9,JBM4S,1ZGHP,QB3GX',NULL,NULL,NULL,'Pinnacle HSA Combined Ex-Test','202201311','EMPEXPORT','TEST_XOE','Feb 23 2021  8:54AM','EPINHSAEXP',NULL,NULL,NULL,'202201311','Feb 15 2021 12:00AM','Dec 30 1899 12:00AM','202201311','1429','','','202201311',dbo.fn_GetTimedKey(),NULL,'us3lKiMEN1004',NULL);
...
Darren Collard      02/04/2022     TekP-2021-12-21-1  Add BASES,BCBS,PLUSP,SLOCA, HSAFC, HSAIC ded codes
...
EXEC dbo._dsi_usp_ExportRipOut_v7_4 @FormatCode = 'EPINHSAEXP', @AllObjects = 'Y', @IsWeb = 'Y'
**********************************************************************************/
BEGIN
    ...
    SET @DedList = 'HSAF,HSAI,HSAFC,HSAIC'; --,HSAER';
    SET @DedListSupp = 'HDH,HDHB,OAP,OAPB,BASES,BCBS,PLUSP,SLOCA';
    ...
    SELECT DISTINCT
        ...
        ,drvEffectiveDate =    CASE WHEN EecEmplStatus = 'T' THEN 
                                    CASE WHEN BdmDedCode IN ('HDH','HDHB','OAP','OAPB','BASES','BCBS','PLUSP','SLOCA') and BdmBenStopDate IS NOT NULL THEN DATEADD(DAY, 30, BdmBenStopDate) 
                                         WHEN BdmDedCode IN ('HDH','HDHB','OAP','OAPB','BASES','BCBS','PLUSP','SLOCA') and BdmBenStopDate IS NULL THEN DATEADD(DAY, 30, BdmBenStatusDate)
                                    END
    ...
    SELECT DISTINCT
        ...
        ,drvEnrollmentEffectiveDate =    CASE WHEN BdmDedCode = 'HSAER' THEN (SELECT MAX(HBDM.BdmBenStartDate) FROM dbo.U_dsi_BDM_EPINHSAEXP HBDM WITH (NOLOCK) WHERE HBDM.BdmEEID = xEEID AND HBDM.BdmCOID = xCOID)
                                        ...
                                        END
        ,drvHDHPCoverageLevel = CASE WHEN BdmDedCode IN ('HDH','HDHB','OAP','OAPB','BASES','BCBS','PLUSP','SLOCA') THEN
    ...
    LEFT JOIN (
                ...
                WHERE PdhDedCode IN ('HSAF','HSAI','HSAFC','HSAIC')
...
--Update Dates
UPDATE dbo.AscExp
    SET expLastStartPerControl = '202201311'
       ,expStartPerControl     = '202201311'
       ,expLastEndPerControl   = '202201311'
       ,expEndPerControl       = '202201311'
       ,expAscpFilename         = '\\us.saas\EW4\EW45\Downloads\V10\Exports\MHCOO\EmployeeHistoryExport\DC_EPINHSAEXP_20220204.txt'
WHERE expFormatCode = 'EPINHSAEXP' AND expExportCode LIKE '%TEST%';
