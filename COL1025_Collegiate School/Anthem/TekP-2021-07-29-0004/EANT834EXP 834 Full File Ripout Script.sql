...
Client ID:      COL1025
Date/time:      2022-03-10 04:45:41.007
Ripout version: 7.4
Export Type:    Web
Status:         Production
Environment:    EWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'EANT834EXP_20220310.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','',NULL,NULL,NULL,'Full File Only','202201319','EMPEXPORT','FULLFILE','Mar  8 2022  3:16PM','EANT834EXP',NULL,NULL,NULL,'202201319','Jan 31 2022 12:00AM','Dec 30 1899 12:00AM','202201171','4881','','','202201171',dbo.fn_GetTimedKey(),NULL,'us3rVaCOL1025',NULL);
...
BEGIN
    ...
    ***************************************************************************************************************/
    ...
    SELECT DISTINCT drvISA05_SenderIDQual = 'ZZ'
        ...
        ,drvQTY02_Quantity1 = (SELECT COUNT(DISTINCT(BdmEEID)) FROM dbo.U_dsi_bdm_EANT834EXP WITH(NOLOCK) WHERE BdmRecType = 'EMP') + (SELECT COUNT(DISTINCT(BdmDepRecID)) FROM dbo.U_dsi_bdm_EANT834EXP WITH(NOLOCK) WHERE BdmRecType = 'DEP')
    ...
    ***************************************************************************************************************/
    ...
    SELECT DISTINCT
        ...
        drvHD00_HealthCoverage = CASE WHEN BdmDedType IN ('MED','OT1') THEN 'HD'
        ...
        ,drvHD02_MaintReasonCode = CASE WHEN BdmDedType IN ('MED') THEN ''
                                     ...
                                     END
        ,drvHD03_InsuranceLineCode =    CASE WHEN BdmDedCode IN ('BMED','BUMED','COMED','CRITI','ACDNT','CI10K','CI1EC','CI1ES','CI1FM','CI20K','CI2EC','CI2ES','CI2FM') THEN 'HLT'
        ...
        ,drvDTP02_DateTimeFormatQual_348 = CASE WHEN BdmDedType IN ('MED','DEN','VIS', 'OT1') THEN 'D8' END
        ,drvDTP03_DateTimePeriod_348 = CASE WHEN BdmDedType IN ('MED','DEN','VIS', 'OT1') THEN dbo.dsi_fnGetMinMaxDates('MAX',BdmBenStartDate, '9/1/2021') END
        -- If drvDTP00_DateTime_349 Populated, then send DTP*349 Segment
        ,drvDTP00_DateTime_349 = CASE WHEN BdmDedType IN ('MED','DEN','VIS', 'OT1') AND BdmBenStopDate IS NOT NULL THEN 'DTP' END
        ,drvDTP01_DateTimeQualifier_349 = CASE WHEN BdmDedType IN ('MED','DEN','VIS', 'OT1') AND BdmBenStopDate IS NOT NULL THEN '349' END
        ,drvDTP02_DateTimeFormatQual_349 = CASE WHEN BdmDedType IN ('MED','DEN','VIS', 'OT1') AND BdmBenStopDate IS NOT NULL THEN 'D8' END
        ,drvDTP03_DateTimePeriod_349 = CASE WHEN BdmDedType IN ('MED','DEN','VIS', 'OT1') THEN BdmBenStopDate END
        -- If drvDTP00_DateTime_303 = 'DTP', then Send DTP*303 Segment
        ,drvDTP00_DateTime_303 = CASE WHEN BdmDedType IN ('MED', 'OT1') THEN '' END
        ,drvDTP01_DateTimeQualifier_303 = CASE WHEN BdmDedType IN ('MED', 'OT1') THEN '303' END
        ,drvDTP02_DateTimeFormatQual_303 = CASE WHEN BdmDedType IN ('MED', 'OT1') THEN 'D8' END
        ,drvDTP03_DateTimePeriod_303 = CASE WHEN BdmDedType IN ('MED', 'OT1') THEN dbo.dsi_fnGetMinMaxDates('MAX',BdmBenStatusDate, @FileMinCovDate) END
