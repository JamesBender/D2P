...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvSort = '' --ISNULL(USESL_EedNeedEOI, 'nope') + ' : ' + ISNULL(USSL_EedNeedEOI, 'nope') --CAST(EecAnnSalary AS VARCHAR)
        ...
        ,drvSPAmtToBeUW =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN 
                            ...
                            END
        ,drvSpouseNameFirst =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                    CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN ConNameFirst END
                                END
        ,drvSpouseNameMiddle =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                    CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN LEFT(ConNameMiddle,1) END
                                END
        ,drvSpouseNameLast =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                    CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN ConNameLast END
                                END
        ,drvRelationshipToEmployee =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                            CASE WHEN USSL_DedCode IS NOT NULL AND ConRelationship = 'DP' AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN 'Domestic Partner' END
                                        END
        ,drvSpouseDateOfBirth =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                    CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN ConDateOfBirth END
                                END
        ,drvSpouseSSN =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                            CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN ConSSN END
                        END
        ,drvSpouseGender =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN
                                    CASE WHEN ConGender IN ('M','F') THEN ConGender ELSE 'M' END
                                END
                            END
        ,drvSpouseEmailAddress =    CASE WHEN USSL_EedNeedEOI = 'Y' THEN
                                        CASE WHEN USSL_DedCode IS NOT NULL AND (ISNULL(USSL_EedNeedEOI, '') = 'Y' OR USSL_EedEOIDesiredAmt > 25000) THEN ConEmailAddr END
                                    END
