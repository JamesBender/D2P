...
Client ID:      GOO1026
Date/time:      2021-12-27 16:28:50.427
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    EWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'EGODNYPORT_20211227.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','',NULL,NULL,NULL,'NYLife Port and Convert -Test','202112319','EMPEXPORT','TEST_XOE','Dec 17 2021  6:11PM','EGODNYPORT',NULL,NULL,NULL,'202112319','Dec 31 2021 12:00AM','Dec 30 1899 12:00AM','202111011','160','','','202111011',dbo.fn_GetTimedKey(),NULL,'us3lKiGOO1026',NULL);
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvEmpIssuedAmountBasicLife = CASE WHEN GLIFP is not null THEN dbo.dsi_fnpadzero((ISNULL(GLIFP_AMT*100,0)),9,0) 
                                            WHEN GLIFE is not null then dbo.dsi_fnpadzero((ISNULL(GLIFE_AMT*100,0)),9,0)  
        ...
        ,drvEmployeeBasicLifeEndDate = CASE WHEN GLIFE IS NOT NULL and GLIFE_BenStopDate IS NOT NULL THEN Convert(CHAR(8),GLIFE_BenStopDate,112) 
                                             WHEN    GLIFP IS NOT NULL and GLIFP_BenStopDate IS NOT NULL THEN Convert(CHAR(8),GLIFP_BenStopDate,112) 
                                                ELSE '00000000' END
        ,drvSpsIssuedAmountBasicLife = CASE WHEN LIFE_REL_SPS IS not null then '000100000' ELSE '000000000' END
        ,drvSpouseBasicLifeEndDate = CASE WHEN GLIFE IS NOT NULL and GLIFE_BenStopDate IS NOT NULL and LIFE_REL_SPS IS not null THEN Convert(CHAR(8),GLIFE_BenStopDate,112) 
                                             WHEN GLIFP IS NOT NULL and GLIFP_BenStopDate IS NOT NULL and LIFE_REL_SPS IS not null THEN Convert(CHAR(8),GLIFP_BenStopDate,112) 
                                                ELSE '00000000' END

        ,drvChildIssuedAmountBasicLife = CASE WHEN LIFE_REL_CHL IS not null then '000100000' ELSE '000000000' END
        ,drvChildBasicLifeEndDate = CASE WHEN GLIFE IS NOT NULL and GLIFE_BenStopDate IS NOT NULL and LIFE_REL_CHL IS not null THEN Convert(CHAR(8),GLIFE_BenStopDate,112) 
                                             WHEN GLIFP IS NOT NULL and GLIFP_BenStopDate IS NOT NULL and LIFE_REL_CHL IS not null THEN Convert(CHAR(8),GLIFP_BenStopDate,112) 
                                                ELSE '00000000' END
        ...
        ,drvClassCodeVoluntaryLife =  CASE WHEN LIFEE is not null or LIFS is not null or LIFC is not null THEN '001' ELSE '000' END
        ...
        ,drvClassCodeVoluntaryADD =  CASE WHEN LIFEE is not null or LIFS is not null or LIFC is not null THEN '001' ELSE '000' END
    ...
    JOIN (Select 
                ...
                LIFE_REL_SPS  = MAX( CASE WHEN bdmdedcode  IN ('GLIFE','GLIFP') and ConRelationship IN ('SPS','DP') THEN '1' END),
                --LIFE_REL_SPS  = MAX( CASE WHEN bdmdedcode  IN ('GLIFE','GLIFP') and ConRelationship IN ('SPS','DP') and ConIsDependent = 'Y' THEN '1' END),
                LIFE_REL_CHL  = MAX( CASE WHEN bdmdedcode  IN ('GLIFE','GLIFP') and ConRelationship IN ('CHL', 'DIS', 'DPC', 'SGR' , 'STC' ) THEN '1' END),
                --LIFE_REL_CHL  = MAX( CASE WHEN bdmdedcode  IN ('GLIFE','GLIFP') and ConRelationship IN ('CHL', 'DIS', 'DPC', 'SGR' , 'STC' ) and ConIsDependent = 'Y' THEN '1' END),
                ...
                BAD  = MAX( CASE WHEN bdmdedcode IN ('GLIFE','GLIFP') THEN 'BAD' END),
                BAD_AMT = MAX(CASE WHEN Bcadedcode IN ('GLIFE','GLIFP') THEN bcabenamtcalc END ),
                BAD_BenStopDate = MAX(CASE WHEN Bcadedcode IN ('GLIFE','GLIFP') THEN bdmbenstopdate END ),
                ADDCode  = MAX( CASE WHEN bdmdedcode IN ('GLIFE','GLIFP') THEN 'AD' END),
                ADDCode_AMT = MAX(CASE WHEN Bcadedcode IN ('GLIFE','GLIFP') THEN bcabenamtcalc END ),
                ADDCode_BenStopDate = MAX(CASE WHEN Bcadedcode IN ('GLIFE','GLIFP') THEN bdmbenstopdate END ),
                ...
                LIFC  = MAX( CASE WHEN bdmdedcode IN ('LIFEC') THEN 'LIFC' END),
                LIFC_AMT = MAX(CASE WHEN Bcadedcode IN ('LIFEC') and bcabenamtcalc is not null THEN bcabenamtcalc ELSE '000100000' END),
                LIFC_BenStopdate = MAX(CASE WHEN Bcadedcode IN ('LIFEC')  THEN bdmbenstopdate END )
