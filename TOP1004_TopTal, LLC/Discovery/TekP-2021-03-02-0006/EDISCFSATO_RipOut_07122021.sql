...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EDISCFSATO_20210712.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Discovery FSA Export-Test','202106159','EMPEXPORT','TEST_XOE','Jul 12 2021  1:28PM','EDISCFSATO',NULL,NULL,NULL,'202106159','Jun 15 2021 12:00AM','Dec 30 1899 12:00AM','202106151','157','eecPayGroup','CRTEAM','202106151',dbo.fn_GetTimedKey(),NULL,'us3cPeTOP1004',NULL);
...
CREATE TABLE [dbo].[U_EDISCFSATO_enTbl] (
    ...
    [PartElectAmt] varchar(8000) NULL,
    ...
    [HDHPCovLevel] varchar(6) NOT NULL,
...
CREATE TABLE [dbo].[U_EDISCFSATO_ptTbl] (
    ...
    [IdIssuingState] varchar(1) NOT NULL,
...
06/21/2021 by AP:
        - Updated amont in the CT record to 2 decimal places.

07/12/2021 by AP:
		- Added MAX start date set to 01/01/current_year.
		- Removed PARK and CMT from the fields.
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,EnrollEffDate = ISNULL((CASE WHEN EedDedCode IN ('FSA', 'DCFSA', 'HSACF', 'HSACI', 'HSAF', 'HSAI', 'CMPK', 'CMTR') THEN REPLACE(CONVERT (CHAR(10), dbo.dsi_fnGetMinMaxDates('MAX', EedBenStartDate, @FileMinCovDate), 101),'/','') END), '')
        ...
        ,ElectionAmtIndicator = CASE WHEN EedDedCode IN ('HSACF', 'HSACI', 'HSAF', 'HSAI') THEN 'PerPay'
                                        WHEN EedDedCode IN ('CMPK', 'CMTR') THEN 'PerMonth' ELSE '' END
    ...
    SELECT DISTINCT
        ...
        ,ContAmt = CAST(CASE WHEN PdhDedCode IN ('DCFSA', 'FSA', 'HSACF', 'HSACI', 'HSAF', 'HSAI', 'HSAER', 'CMPK', 'CMTR') AND PdhEECurAmt > 0 THEN CAST(PdhEECurAmt AS DECIMAL(10,2)) END AS VARCHAR)
        ,AmtType = 'Actual'
        ,TaxYear = 'Current'
        ,drvInitialSort = LTRIM(RTRIM(EepSSN))
        ,drvSubSort = '3'
