...
Client ID:      UST1001
Date/time:      2023-05-26 10:21:20.420
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    E14
Server:         E1D314DB01
Database:       ULTIPRO_UST
Web Filename:   UST1001_18510_EEHISTORY_ESTMCADEMO_ExportCode_YYYYMMDD_HHMMSS.txt
ExportPath:    
TestPath:      
...
IF OBJECT_ID('dsi_vwESTMCADEMO_Export') IS NOT NULL DROP VIEW [dbo].[dsi_vwESTMCADEMO_Export];
GO
IF OBJECT_ID('dsi_sp_BuildDriverTables_ESTMCADEMO') IS NOT NULL DROP PROCEDURE [dbo].[dsi_sp_BuildDriverTables_ESTMCADEMO];
GO
IF OBJECT_ID('U_ESTMCADEMO_PEarHist') IS NOT NULL DROP TABLE [dbo].[U_ESTMCADEMO_PEarHist];
GO
IF OBJECT_ID('U_ESTMCADEMO_File') IS NOT NULL DROP TABLE [dbo].[U_ESTMCADEMO_File];
GO
IF OBJECT_ID('U_ESTMCADEMO_EEList') IS NOT NULL DROP TABLE [dbo].[U_ESTMCADEMO_EEList];
GO
IF OBJECT_ID('U_ESTMCADEMO_drvTbl') IS NOT NULL DROP TABLE [dbo].[U_ESTMCADEMO_drvTbl];
GO
...
/*08*/ DECLARE @FileName varchar(1000) = 'ESTMCADEMO_20230526.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','',',GTJ6M,M6S5A,GRPSV,GRPR1',NULL,NULL,NULL,'CultureAmp Demographics -Test','202305259','EMPEXPORT','TEST_XOE','Nov  9 2022 10:20AM','ESTMCADEMO',NULL,NULL,NULL,'202305259','Nov  9 2022 12:00AM','Dec 30 1899 12:00AM','202305251','1112','','','202305251',dbo.fn_GetTimedKey(),NULL,'us3kRiUST1001',NULL);
...
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('ESTMCADEMO','ExportPath','V',NULL);
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('ESTMCADEMO','Testing','V','Y');
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('ESTMCADEMO','TestPath','V',NULL);
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('ESTMCADEMO','UseFileName','V','Y');
...
-----------
-- Create table U_ESTMCADEMO_drvTbl
-----------

IF OBJECT_ID('U_ESTMCADEMO_drvTbl') IS NULL
CREATE TABLE [dbo].[U_ESTMCADEMO_drvTbl] (
    [drvEEID] char(12) NULL,
    [drvCoID] char(5) NULL,
    [drvDepRecID] varchar(12) NULL,
    [drvName] varchar(6000) NULL,
    [drvPreferredName] varchar(100) NULL,
    [drvEmployeeID] varchar(5) NULL,
    [drvEmail] varchar(50) NULL,
    [drvDateOfBirth] varchar(30) NULL,
    [drvStartDate] datetime NULL,
    [drvEndDate] varchar(10) NULL,
    [drvLanguage] varchar(2) NOT NULL,
    [drvManagerEmail] varchar(256) NULL,
    [drvManagerName] varchar(770) NULL,
    [drvPerformanceRating] varchar(45) NULL,
    ...
    [drvEmployment] varchar(9) NULL,
    [drvHourlySalaried] varchar(8) NULL,
    [drvGrade] char(6) NULL,
    [drvPeopleManager] varchar(22) NOT NULL,
    [drvUnion] char(12) NULL,
    [DrvWorkState] varchar(2) NULL,
    [drvEnthnecity] varchar(45) NULL,
    [drvMilitary] varchar(3) NOT NULL,
    [drvDisabilityCodfe] varchar(40) NOT NULL,
    [drvOrg2Code] varchar(10) NULL,
    [drvOrg2] varchar(25) NULL,
    [drvOrg3Code] varchar(10) NULL,
    [drvOrg3] varchar(25) NULL,
    [drvOrg4Code] varchar(10) NULL,
    [drvOrg4] varchar(25) NULL
);

-----------
-- Create table U_ESTMCADEMO_EEList
-----------

IF OBJECT_ID('U_ESTMCADEMO_EEList') IS NULL
CREATE TABLE [dbo].[U_ESTMCADEMO_EEList] (
    [xCOID] char(5) NULL,
    [xEEID] char(12) NULL
);

-----------
-- Create table U_ESTMCADEMO_File
-----------

IF OBJECT_ID('U_ESTMCADEMO_File') IS NULL
CREATE TABLE [dbo].[U_ESTMCADEMO_File] (
...
-----------
-- Create table U_ESTMCADEMO_PEarHist
-----------

IF OBJECT_ID('U_ESTMCADEMO_PEarHist') IS NULL
CREATE TABLE [dbo].[U_ESTMCADEMO_PEarHist] (
...
Marie Waters        05/26/2023     TekP-2022-09-14-06 Updated Manager Name,  Performance Rating, People Manager and Disability Code fields
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvName = dbo.dsi_fnRemoveChars('.,/-', TRIM(EepNameFirst) + ' ' + TRIM(eepNameLast) + ' ' 
        ...
        ,drvStartDate = CASE WHEN EecDateOfOriginalHire <> EecDateOfLastHire THEN CONVERT(VARCHAR(10), EecDateOfLastHire, 23) ELSE EecDateOfOriginalHire END
        ...
        ,drvManagerName = dbo.dsi_fnlib_GetSupervisorField('', EecEEID, 'NameFirst')  + ' ' + dbo.dsi_fnlib_GetSupervisorField('', EecEEID, 'NameLast' ) + ' ' +ISNULL(dbo.dsi_fnlib_GetSupervisorField('', EecEEID, 'Suffix'),'')
        ,drvPerformanceRating = CASE WHEN ISNULL(EecReviewPerfRating, 'Z') <> 'Z' THEN RatingCodeDesc ELSE '' END
        ...
        ,drvPeopleManager = CASE WHEN dbo.dsi_fnlib_IsSupervisor(xEEID) = 'Y' THEN 'People Manager' ELSE 'Individual Contributor' END
        ,drvUnion = EecUnionNational
        ,DrvWorkState = LEFT(EecStateSUI,2)
        ,drvEnthnecity = Eth.CodDesc
        ,drvMilitary = CASE WHEN eepMilitary = 'Y' THEN 'Yes' ELSE 'No' END
        ,drvDisabilityCodfe = CASE WHEN EepIsDisabled = 'Y' THEN 'Has a disability (or previously had one)'
                                WHEN EepIsDisabled = 'N' THEN  'Does not have a disability' 
                                WHEN EepIsDisabled = 'D' THEN   'Does not wish to answer' 
                               ELSE 'Not informed/unknown'
                            END
    ...
    LEFT JOIN dbo.PmRatingScaleLevel WITH (NOLOCK)
        ON EecReviewPerfRating = RatingCode
;
    --==========================================
    -- Set FileName
    --==========================================
...
**********************************************************************************/
GO
CREATE VIEW dbo.dsi_vwESTMCADEMO_Export AS 
    SELECT TOP 200000000 Data FROM dbo.U_ESTMCADEMO_File WITH (NOLOCK)
