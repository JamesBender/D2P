...
06/05/2015 - NCO - Added a bit of code to allow a blank viewable menu with no aspx page as there was an issue
                 - with the deployment in SAAS that needed this mod
*/
...
/*
Utility function to return the RerID of a core menu path.
Used by utility procedures U_WebCustoms_AddMenuElement and U_WebCustoms_RemoveMenuElement.

Modification History:
06/16/11 - DCW - Initial version
04/10/13 - DCW - Rename from "U_FN_WebCustoms_GetCoreRerIDFromMenuPath" to "U_FN_WebCustoms_GetRerIDFromMenuPath", and let it handle custom elements in the path as well (used to expect them always to be core)
*/
...
BEGIN
	...
	WHILE (@ProcessID <= @MaxID) BEGIN
		...
		WHERE rerParentID = @PrevRerID
		  AND @ItemLabel = CASE eleType		-- Core ElementTypes are defined in Hrms_GlobalData.dbo.RbsElementTypes
...
/*
Utility Procedure to insert a custom menu or tab element under a core one.

Modification History:
06/16/11 - DCW - Initial version
06/01/12 - NCO - Added support for "Action" element types (the "Things I Can Do" menu items)
04/10/13 - DCW - Change/improve handling for custom elements in menu paths. Removed IsCustom and ProdCode parameters, changed call to U_FN_WebCustoms_GetCoreRerIDFromMenuPath to be U_FN_WebCustoms_GetRerIDFromMenuPath instead.
04/10/13 - DCW - Add call to U_WebCustoms_EnsureRbsPageCoMap
04/02/14 - DCW - Handle NULL RightsMask after custom element creation, sometimes happens.
04/03/14 - DCW - Add ProdCodeOverride parameter
*/
CREATE PROCEDURE dbo.U_WebCustoms_AddMenuElement (
	...
	@Location	  CHAR(1) = 'M',  -- Allow setting of location
...
'
	...
	IF @ElementType = 'MENU' AND ISNULL(@AspxFilename,'') = ''
	BEGIN
...
Has to be directly under a core one.
Example path: Employee Admin > My Employees > (Employee Popup) > Jobs > Stock Grants
In this case, "Stock Grants" in the custom item, everything else before that is core.
See function dbo.U_FN_WebCustoms_GetCoreRerIDFromMenuPath for more.

Modification History:
06/16/11 - DCW - Initial version
04/10/13 - DCW - Add handling for "Action" element types. Add handling for custom elements in menu path, mainly by changing call to U_FN_WebCustoms_GetCoreRerIDFromMenuPath to be U_FN_WebCustoms_GetRerIDFromMenuPath instead.
*/
...
'<DATA>
  <CUSTOM>
	...
	IF (CHARINDEX(',',@CustomPath) > 0) BEGIN
		PRINT 'Error: U_WebCustoms_RemoveMenuElement: Commas not allowed in the CustomPath parameter'
		RETURN
	END
	...
	IF (@MaxRecID = 0) BEGIN
		PRINT 'Error: U_WebCustoms_RemoveMenuElement: Error parsing CustomPath parameter. Zero elements retrieved.'
		RETURN
	END
	
	IF (@MaxRecID = 1) BEGIN
		PRINT 'Error: U_WebCustoms_RemoveMenuElement: CustomPath parameter should have more than one item in the path'
		RETURN
	END
	...
	IF (LEN(@ParentPath) = 0) BEGIN
		PRINT 'Error: U_WebCustoms_RemoveMenuElement: Some problem re-establishing ParentPath. Came up blank.'
		RETURN
	END
	...
	IF (ISNULL(@ParentID,0) <= 0) BEGIN
		PRINT 'Error: U_WebCustoms_RemoveMenuElement: Parent RerID could not be found'
		RETURN
	END
	...
	IF (@CustomRerID IS NULL) BEGIN
		PRINT 'Info: U_WebCustoms_RemoveMenuElement: Custom menu element not found. No need to remove.'
	END ELSE BEGIN
		IF (@CustomElementType NOT IN ('Menu','Tab','Action')) BEGIN
			PRINT 'Error: U_WebCustoms_RemoveMenuElement: Only Menu, Tab, or Action custom element types are supported. ElementType=[' + @CustomElementType + ']'
		END ELSE BEGIN
...
-- Client:		  Amcor Corporate Shared Services
-- Date:		  5/8/2019
-- Request:		  SR-2019-00236483
-- Purpose:		  Setup assistance for Auto Assign Web Portal Manager Roles eModule Solution - Increase in Hours
-- Last Modified: 
-----------------------------------------------------------------------------------------------------------------------
EXEC U_WebCustoms_RemoveMenuElement 'System Configuration > RefJobDetail'
EXEC U_WebCustoms_RemoveMenuElement 'System Configuration > SubdomDetail'
...
-----------------------------------------------------------
--Initial Author:		Steven Bonkosky(SB)
--Client        :		AREVA Enterprises, Inc (fka AREVA NP, Inc.)(FRA1000)
--Date          :		6/2013
--Purpose       :		CS-2013-71645 - AREVA Enterprises, Inc 

--Ultipro Version: Ultipro

------------------------------------------------------------

UPDATE JMSJobDefSource
SET jobDefSource = '<ProcessInfo>
  <Description>This is a test description</Description>
  <Document>This is a test document</Document>
  <ExecutionDetails>
    <ExecutionStep Order="1">
      <TypeName>EmployeeSetup</TypeName>
      <MethodName>SetJobsUserDefined</MethodName>
    </ExecutionStep>
  </ExecutionDetails>
</ProcessInfo>'
WHERE jobDefCode = 'JobsUserDefined'

select * from JMSJobDefSource WHERE jobDefCode = 'JobsUserDefined'
