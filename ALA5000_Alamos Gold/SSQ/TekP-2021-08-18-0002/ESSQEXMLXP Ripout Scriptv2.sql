...
Client ID:      ALA5000
Date/time:      2023-04-28 12:05:43.830
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    T12
...
GO
IF OBJECT_ID('U_ESSQEXMLXP_AuditFields') IS NOT NULL DROP TABLE [dbo].[U_ESSQEXMLXP_AuditFields];
GO
IF OBJECT_ID('U_ESSQEXMLXP_Audit') IS NOT NULL DROP TABLE [dbo].[U_ESSQEXMLXP_Audit];
GO
...
INSERT INTO [dbo].[AscDefF] (AdfFieldNumber,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType,AdfExpression,AdfForCond) VALUES ('5','ESSQEXMLXPZ0','50','H','03','5',NULL,'Header',NULL,NULL,'"<frequency>70</frequency>"','(''DA''=''T'')');
...
INSERT INTO [dbo].[AscDefF] (AdfFieldNumber,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType,AdfExpression,AdfForCond) VALUES ('14','ESSQEXMLXPZ0','50','D','25','14',NULL,'Address City Open',NULL,NULL,'"</addressLine3><city>"','(''DA''=''T'')');
...
INSERT INTO [dbo].[AscDefF] (AdfFieldNumber,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType,AdfExpression,AdfForCond) VALUES ('16','ESSQEXMLXPZ0','50','D','25','16',NULL,'Address City Close',NULL,NULL,'"</city>"','(''DA''=''T'')');
...
/*08*/ DECLARE @FileName varchar(1000) = 'ESSQEXMLXP_20230428.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'Null','N','8GWCL,OAXLN,8GW4G,NMZYE,NMZZE,8GVSQ,TPHJ0,8GVZ9,OCGNP,NN000,NN00X',NULL,NULL,NULL,'SSQ Benefits XML - BW Wed 8am','202208249','EMPEXPORT','SCH_ESSQEX','Nov 24 2021  8:26AM','ESSQEXMLXP',NULL,NULL,NULL,'202304199','Nov 19 2021  5:29AM','Nov 19 2021  5:29AM','202303161','14871','','','202208241',dbo.fn_GetTimedKey(),NULL,'ULTI',NULL);
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','',NULL,NULL,NULL,'SSQ Benefits XML Export-Test','202304219','EMPEXPORT','TEST_XOE','Nov  1 2022  9:14AM','ESSQEXMLXP',NULL,NULL,NULL,'202304219','Oct 29 2022 12:00AM','Dec 30 1899 12:00AM','202304141','15915','','','202304141',dbo.fn_GetTimedKey(),NULL,'us3cPeALA5000',NULL);
...
-----------
-- Create table U_ESSQEXMLXP_Audit
-----------

IF OBJECT_ID('U_ESSQEXMLXP_Audit') IS NULL
CREATE TABLE [dbo].[U_ESSQEXMLXP_Audit] (
...
-----------
-- Create table U_ESSQEXMLXP_AuditFields
-----------

IF OBJECT_ID('U_ESSQEXMLXP_AuditFields') IS NULL
CREATE TABLE [dbo].[U_ESSQEXMLXP_AuditFields] (
...
CREATE TABLE [dbo].[U_ESSQEXMLXP_drvTbl_Dependent] (
    ...
    [drvConNameLast] varchar(8000) NULL,
    [drvConNameFirst] varchar(8000) NULL,
...
CREATE TABLE [dbo].[U_ESSQEXMLXP_drvTbl_ParticipantGroupClose] (
    ...
    [drvDepRecID] varchar(12) NULL,
    [drvSort] varchar(16) NULL,
    [drvParticipantGroupClose] varchar(28) NULL
);
...
LeaK/INSPYR            04/03/2023     TekP-2023-03-22-01        Changed frequency from 10 to 70
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvSort = '2 ' + xEEID + ' D'
        -- standard fields above and additional driver fields below
        ,drvAddressLine1 = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(EepAddressLine1, 'ñ', 'n'), 'á', 'a'), 'é','e'), 'í', 'i'), 'ó', 'o'), 'ú','u'), 'è', 'e'), 'à', 'a'), 'ô', 'o'), 'ç', 'c'), 'ë', 'e'), '½', '1/2')
  ...
  SELECT DISTINCT
        ...
        ,drvStatusStartDate =      CASE WHEN EecEmplStatus = 'T' THEN EecDateOfTermination
                                ...
                                WHEN EecEmplStatus = 'A' AND EshEmplStatus = 'L' and EshIsCurrent  = 'N' THEN EshStatusStopDate
    ...
    JOIN dbo.vw_int_EmpComp WITH (NOLOCK)
        ...
        AND EecCoID = xCoID
    LEFT JOIN (
                SELECT EshEEID, EshCOID, EshStatusStartDate , EshStatusStopDate, EshEmplStatus , EshIsCurrent 
                FROM (
                        SELECT EshEEID, EshCOID, EshStatusStartDate, EshStatusStopDate, EshEmplStatus, EshIsCurrent, ROW_NUMBER() OVER (PARTITION BY EshEEID, EshCOID ORDER BY EshStatusStartDate DESC) AS RN
                        FROM dbo.EmpHStat WITH (NOLOCK)  WHERE EshEmplStatus = 'L'  ) AS LOA_Dates 
                WHERE RN = 1 ) AS EshDates 
        ON EshEEID = xEEID 
        AND EshCOID = xCOID
   -- LEFT JOIN (
   --             SELECT AudEEID, AudCOID,  LeaveStartDate, LeaveStopDate, CurrentStatus, PreviousStatus
   --             FROM (
            --          Select audkey1value AS AudEEID 
            --                ,audkey2value AS AudCOID
            --                ,MAX(CASE WHEN audOldValue = 'A' and audnewvalue = 'L'  THEN auddatetime END) AS LeaveStartDate
            --                ,MAX(CASE WHEN audOldValue = 'L' and audnewvalue = 'A'  THEN auddatetime END)  AS LeaveStopDate
            --                ,Max(audNewvalue) as CurrentStatus
            --                ,MAX( audoldvalue) as PreviousStatus        
            --            FROM dbo.vw_AuditData WITH (NOLOCK) 
            --            WHERE audtablename = 'EmpComp' 
            --            and audfieldname = 'EecEmplStatus' 
            --            and (audOldValue IN ('A', 'L') AND audnewvalue IN ('A', 'L'))    
            --            --and audkey1value  = 'CRYUCM01D030'
            --            Group by audkey1value, audkey2value
            --            )Aud 
         --) AS EshDates 
   --     ON  AudEEID = xEEID 
   --     AND AudCOID = xCOID
    ...
    JOIN dbo.Contacts WITH (NOLOCK)
        ...
        AND ConRelationship IN ('CHL','STC','COM','SPS')
    ...
    SELECT DISTINCT
        ...
        ,drvVolume = CASE WHEN BdmDedCode IN ('25127','25117','25116','25118','25119','25112','25122','25123','25124','2512Q','25126','25419','25412','25416','25417','25418','25422','25423','25424','25427','2542Q','25426') THEN FORMAT(EedBenAmt, '#0') END
        --,drvVolume = CASE WHEN BdmDedCode IN ('25127','25117','25116','25118','25119','25112','25122','25123','25124','2512Q','25126','25419','25412','25416','25417','25418','25422','25423','25424','25427','2542Q','25426') THEN FORMAT(EedBenAmt, '#0.00') END
    ...
    SELECT DISTINCT
        ...
        ,drvSort = '2 ' + xEEID + ' K'
        -- standard fields above and additional driver fields below
        ,drvParticipantGroupClose = '</coverages>' + CHAR(13) + CHAR(10) + '</participant>'
...
--Update Dates
UPDATE dbo.AscExp
    SET expLastStartPerControl = '202304141'
       ,expStartPerControl     = '202304141'
       ,expLastEndPerControl   = '202304219'
       ,expEndPerControl       = '202304219'
WHERE expFormatCode = 'ESSQEXMLXP' and expexportcode = 'TEST_XOE';
