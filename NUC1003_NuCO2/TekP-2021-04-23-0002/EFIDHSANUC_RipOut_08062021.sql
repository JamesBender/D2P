...
--INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('""','1','(''SS''=''F'')','EFIDHSANUCZ0','7','H','01','1',NULL,'TEST FILE IDENTIFIER',NULL,NULL);
...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EFIDHSANUC_20210806.txt';
...
INSERT INTO [dbo].[U_dsi_Configuration] (FormatCode,CfgName,CfgType,CfgValue) VALUES ('EFIDHSANUC','Testing','V','N');
...
07/14/2021 by AP:
        - Fixed length of drvAmount to 13 with leading 0's.

07/29/2021 by AP:
        - Config for TEST set to N.

08/06/2021 by AP:
		- Removed TESTEDT header.
   ...
   -- Working Table - PDedHist
    ...
    SELECT DISTINCT
        ...
        ,drvSOURCE = CASE WHEN PdhSource IN ('HSAEMP') THEN '01' --HSA EMPLOYEE
                     ...
                     END
        ,drvAMOUNT = CASE WHEN PdhEECurAmt <> 0.00 THEN CONVERT(VARCHAR(13),PdhEECurAmt)
                            END
    ...
    Update U_EFIDHSANUC_drvTbl_19H set drvAmount  =  RIGHT('0000000000000' + CONCAT(LEFT(REPLACE(drvAmount,'.',''),(LEN( REPLACE(drvAmount,'.','')) -1)),CASE WHEN RIGHT(drvAmount,1) = '1' THEN 'A'
