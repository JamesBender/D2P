...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"drvScheduledDeduction"','40','(''SS''=''F'')','EPAYFLEXELZ0','8','D','10','531',NULL,'Scheduled_Deduction',NULL,NULL);
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','',NULL,NULL,NULL,NULL,'PayFlex Eligibility Expo-Test','202105059','EMPEXPORT','TEST_XOE','May  5 2021 10:58AM','EPAYFLEXEL',NULL,NULL,NULL,'202105059','May  5 2021 12:00AM','Dec 30 1899 12:00AM','202104011','221','','','202104011',dbo.fn_GetTimedKey(),NULL,'RVAS',NULL);
...
CREATE TABLE [dbo].[U_EPAYFLEXEL_drvTbl] (
    ...
    [drvCopayCodeMedical] varchar(2) NOT NULL,
    [drvCopayCodeDental] varchar(2) NOT NULL,
    [drvCopayCodeVision] varchar(2) NOT NULL,
    [drvCopayCodeRX] varchar(2) NOT NULL,
    ...
    [drvAnnualDeduction] varchar(8) NULL,
    [drvScheduledDeduction] varchar(8) NULL,
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvMember_Number = EecEmpNo --EepSsn
        ...
        ,drvControl = '0170546' --'0170551'
        ,drvSuffix = CASE
                        WHEN bdmdedcode IN ('DPFSA', 'HFSA') THEN '010'
                        WHEN bdmdedcode  IN ('HFSAL', 'HSAEE', 'HSAEC', 'HSAFC', 'HSAFM') THEN '011'
                     END
        ,drvAccount = '000'
        ...
        ,drvAccount_Type = CASE
                            WHEN bdmdedcode = 'DPFSA' THEN '4'
                            WHEN bdmdedcode = 'HFSA' THEN '5'
                            WHEN bdmdedcode = 'PARK1' THEN '7'
                            WHEN bdmdedcode = 'TRAN' THEN '8' 
                            WHEN bdmdedcode = 'HFSAL' THEN '11'
                            WHEN bdmdedcode IN ('HFSAL', 'HSAEE', 'HSAEC', 'HSAFC', 'HSAFM') THEN '16'
                           END
        ,drvElection_Effective_Date = CASE WHEN CAST(bdmStartDate AS DATE) > '01/01/2021' THEN BdmBenStartDate ELSE '01/01/2021' END --bdmBenStartDate
        ,drvAnnualDeduction = CASE WHEN BdmDedCode IN ('DPFSA', 'HFSAL', 'HFSA') THEN CAST(CAST(BdmEEGoalAmt AS DECIMAL(5,2)) AS VARCHAR) END
        ,drvScheduledDeduction = CASE WHEN PdhDedCode IN ('DPFSA', 'HFSAL', 'HFSA') THEN CAST(CAST(PdhEECurAmt AS DECIMAL(5,2)) AS VARCHAR) END
        ,drvCommuter_Transit = CASE WHEN BdmDedCode = 'TRAN' THEN 'TT' ELSE '' END
        ,drvCommuter_Parking = CASE WHEN BdmDedCode = 'PARK1' THEN 'TT' ELSE '' END
