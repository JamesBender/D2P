...
Client ID:      STR1015
Date/time:      2023-02-02 16:45:00.703
Ripout version: 7.4
Export Type:    Web
Status:         Testing
Environment:    NWP
...
/*08*/ DECLARE @FileName varchar(1000) = 'EMHTDLFEXP_20230202.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FilePath) + LTRIM(RTRIM(@FileName)),NULL,'','','4I21S,4HYM8,FETK8,4I1Q0,4HYZM,4HX8S',NULL,NULL,NULL,'MoO S/LTD B/V Life Expor-Test','202301069','EMPEXPORT','TEST_XOE','Jul 19 2022  8:48AM','EMHTDLFEXP',NULL,NULL,NULL,'202301069','Jul 14 2022 11:39AM','Jul 14 2022 11:39AM','202212081','569','','','202212081',dbo.fn_GetTimedKey(),NULL,'ULTI',NULL);
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvEffectiveDateVolLifeEE =    CASE WHEN LIFEE_DedCode IS NOT NULL AND LIFEE_BenStatus = 'T' THEN LIFEE_BenStopDate
        ...
        ,drvProductCategoryVolLifeSps = CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') THEN '4' END
        ,drvEffectiveDateVolLifeSps =    CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') AND LIFES_DedCode = 'T' THEN LIFES_BenStopDate
                                        WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') AND LIFES_BenStatus <> 'T' THEN LIFES_BenStartDate
                                        END
        ,drvEligibilityEventVolLifeSps =    CASE WHEN LIFES_DedCode IS NOT NULL AND LIFES_BenStatus = 'T' AND ConRelationship IN ('SPS' ,'DP') THEN 'TM'
                                            ...
                                            END
        ,drvPlanIDVolLifeSps = CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') THEN 'ETL0CSPVAL' END
        ,drvFamCoverageIndVolLIfeSps = CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') THEN 'C' END
        ,drvApprovedAmtEffDteVolLfeSps = CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') THEN LIFES_BenStartDate END
        ,drvApprovedAmountVolLIfeSps = CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') THEN FORMAT(LIFES_AMT, '0000000000') END
        
        -- 1140 - Voluntary Term Life Coverage Dependent
        ,drvProductCategoryVolLifeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN '5' END
        ,drvEffectiveDateVolLifeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_DedCode = 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN LIFEC_BenStopDate
                                           WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_BenStatus <> 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN LIFEC_BenStartDate
                                        END
        ,drvEligibilityEventVolLifeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_BenStatus = 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN 'TM'
                                              WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_BenStatus <> 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN 'EN'
                                             END
        ,drvPlanIDVolLifeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC')THEN 'ETL0CDPVAL' END
        ,drvFamCoverageIndVolLIfeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN 'C' END
        ,drvApprovedAmtEffDteVolLfeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN LIFEC_BenStartDate END
        ,drvApprovedAmountVolLIfeDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN FORMAT(LIFEC_AMT, '0000000000') END

         -- 1195 Voluntary AD&D Coverage Employee
        ,drvProductCategoryVolAddEE = CASE WHEN LIFEE_DedCode IS NOT NULL THEN 'c' END
        ,drvEffectiveDateVolLifeAddEE =  CASE WHEN LIFEE_DedCode IS NOT NULL AND LIFEE_DedCode = 'T' THEN LIFEE_BenStopDate
                                              WHEN LIFEE_DedCode IS NOT NULL AND LIFEE_BenStatus <> 'T' THEN LIFEE_BenStartDate
                                           END
        ,drvEligibilityEvtVolLifeAddEE = CASE WHEN LIFEE_DedCode IS NOT NULL AND LIFEE_BenStatus = 'T' THEN 'TM'
                                              WHEN LIFEE_DedCode IS NOT NULL AND LIFEE_BenStatus <> 'T' THEN 'EN'
                                            END
        ,drvPlanIDVolAddEE =            CASE WHEN LIFEE_DedCode IS NOT NULL THEN 'BTA0CEEVAL' END 
        ,drvFamilyCoverageIndVolAddEE = CASE WHEN LIFEE_DedCode IS NOT NULL THEN 'C' END
        ,drvApprovedAmtEffDteVolAddEE = CASE WHEN LIFEE_DedCode IS NOT NULL THEN LIFEE_BenStartDate END
        ,drvApprovedAmountVolAddEE =    CASE WHEN LIFEE_DedCode IS NOT NULL THEN FORMAT(LIFEE_AMT, '0000000000') END

        -- 1250 Voluntary AD&D Coverage Spouse
        ,drvProductCategoryVolAddSps =     CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') THEN 'e' END
        ,drvEffectiveDateVolLifeAddSps =   CASE WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') AND LIFES_DedCode = 'T' THEN LIFES_BenStopDate
                                                WHEN LIFES_DedCode IS NOT NULL AND ConRelationship IN ('SPS' ,'DP') AND LIFES_BenStatus <> 'T' THEN LIFES_BenStartDate
                                           END
        ,drvEligibilityEvtVolLfeAddSps = CASE WHEN LIFES_DedCode IS NOT NULL AND LIFES_BenStatus = 'T'  AND ConRelationship IN ('SPS' ,'DP') THEN 'TM'
                                              WHEN LIFES_DedCode IS NOT NULL AND LIFES_BenStatus <> 'T'  AND ConRelationship IN ('SPS' ,'DP') THEN 'EN'
                                            END
        ,drvPlanIDVolAddSps =            CASE WHEN LIFES_DedCode IS NOT NULL  AND ConRelationship IN ('SPS' ,'DP') THEN 'BTA0CSPVAL' END
        ,drvFamilyCoverageIndVolAddSps = CASE WHEN LIFES_DedCode IS NOT NULL  AND ConRelationship IN ('SPS' ,'DP') THEN 'C' END
        ,drvApprovedAmtEffDteVolAddSps = CASE WHEN LIFES_DedCode IS NOT NULL  AND ConRelationship IN ('SPS' ,'DP') THEN LIFES_BenStartDate END
        ,drvApprovedAmountVolAddSps = CASE WHEN LIFES_DedCode IS NOT NULL  AND ConRelationship IN ('SPS' ,'DP') THEN FORMAT(LIFES_AMT, '0000000000') END

        -- 1305 Voluntary AD&D Coverage Dependent
        ,drvProductCategoryVolAddDep = CASE WHEN LIFEC_DedCode IS NOT NULL  AND ConRelationship IN ('CHL' ,'DPC') THEN 'd' END
        ,drvEffectiveDateVolLifeAddDep =  CASE WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_DedCode = 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN LIFEC_BenStopDate
                                               WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_BenStatus <> 'T'AND ConRelationship IN ('CHL' ,'DPC') THEN LIFEC_BenStartDate
                                            END
        ,drvEligibilityEvtVolLfeAddDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_BenStatus = 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN 'TM'
                                              WHEN LIFEC_DedCode IS NOT NULL AND LIFEC_BenStatus <> 'T' AND ConRelationship IN ('CHL' ,'DPC') THEN 'EN'
                                            END
        ,drvPlanIDVolAddDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN 'BTA0CDPVAL' END
        ,drvFamilyCoverageIndVolAddDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN 'C' END
        ,drvApprovedAmtEffDteVolAddDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN LIFEC_BenStartDate END
        ,drvApprovedAmountVolAddDep = CASE WHEN LIFEC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') THEN FORMAT(LIFEC_AMT, '0000000000') END
        ...
        ,drvProductCategoryBasicDepLfe = CASE WHEN  ( GLIFC_DedCode IS NOT NULL  AND ConRelationship IN ('CHL' ,'DPC')) THEN '6' END
        ,drvEffectiveDateBasicDepLife = CASE WHEN GLIFC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') AND GLIFC_BenStatus = 'T' THEN GLIFC_BenStopDate
                                             WHEN GLIFC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') AND GLIFC_BenStatus <> 'T' THEN GLIFC_BenStartDate
                                        END
        ,drvElligbilityEvtBasicDepLife = CASE WHEN GLIFC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') AND GLIFC_BenStatus = 'T' THEN 'TM'
                                        WHEN GLIFC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') AND GLIFC_BenStatus <> 'T' THEN 'EN'
                                        END
        ,drvPlanIDBasicDepLife = CASE WHEN  (GLIFC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC')) THEN 'LTL00NCDEP' END
        ,drvFamilyCoveIndBasicDepLife = CASE WHEN  (GLIFC_DedCode IS NOT NULL AND ConRelationship IN ('CHL' ,'DPC') ) THEN 'C' END
    ...
    JOIN (
            SELECT BdmRecType, BdmCOID, BdmEEID, BdmDepRecID
                ...
                ,MAX(CASE WHEN BdmDedCode = 'GLFEX' THEN BdmStopDate END) AS GLFEX_BenStopDate
                ,MAX(CASE WHEN BdmDedCode = 'GLIFS' THEN BdmStopDate END) AS GLIFS_BenStopDate
                ,MAX(CASE WHEN BdmDedCode = 'GLIFC' THEN BdmStopDate END) AS GLIFC_BenStopDate
                ,MAX(CASE WHEN BdmDedCode = 'LIFEC' THEN BdmStopDate END) AS LIFEC_BenStopDate
                ,MAX(CASE WHEN BdmDedCode = 'LIFEE' THEN BdmStopDate END) AS LIFEE_BenStopDate
                ,MAX(CASE WHEN BdmDedCode = 'LIFES' THEN BdmStopDate END) AS LIFES_BenStopDate
