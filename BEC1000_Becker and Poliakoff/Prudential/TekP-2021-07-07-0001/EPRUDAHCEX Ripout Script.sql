...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"1.0"','6','(''DA''=''T~'')','EPRUDAHCEXZ0','50','H','01','6',NULL,'SchemaVersionIdentifier',NULL,NULL);
...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"MOBILE"','39','(''DA''=''T~'')','EPRUDAHCEXZ0','50','D','10','39',NULL,'Associate Telephone Type Code 1',NULL,NULL);
...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"drvDependentId"','4','(''UA''=''T~'')','EPRUDAHCEXZ0','50','D','40','4',NULL,'AssociateID',NULL,NULL);
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvProductPlanCode =    CASE WHEN BdmDedCode IN('CRIL1','CRIL2','CRLC1','CRLC2','CRLS1','CRLS2') THEN '4982'
                                ...
                                END
        ,drvCoverageAmount = FORMAT(CASE WHEN BdmDedCode IN ('CRIL1','CRIL2','CRLC1','CRLC2','CRLS1','CRLS2') /*AND BdmEEAmt IS NOT NULL*/ THEN eedBenAmt ELSE 0.00 END, '#0.00')
    ...
    JOIN dbo.EmpDed WITH (NOLOCK)
        ...
        AND (EedDepRecID = BdmDepRecID OR (EedDepRecID IS NULL AND BdmDepRecID IS NULL))    
    WHERE EecEmplStatus <> 'T' OR (EecEmplStatus = 'T' AND EecDateOfTermination BETWEEN DATEADD(DAY, -30, @EndDate) AND @EndDate)
    ;
    ---------------------------------
    ...
    SELECT DISTINCT
        ...
        -- standard fields above and additional driver fields below
        ,drvAssociateId = RTRIM(EecEmpNo) 
        ,drvDependentId = RTRIM(EecEmpNo) + FORMAT(ConDepNo, '00')
        ,drvDependentRelationship =    CASE WHEN ConRelationship IN ('DP') THEN 'DOMESTIC PARTNER'
                                    ...
                                    END
        ,drvDependentSSN = CASE WHEN ConSSN NOT IN ('123456789') THEN ConSSN END
    ...
    JOIN (
            ...
            SELECT DISTINCT ConEEID
                ...
                ,ConNameLast
                ,ConRelationship
                ,CASE WHEN ConSSN NOT IN ('123456789') THEN ConSSN ELSE NULL END
                ,ConAddressLine1
                ,ConAddressLIne2
                ,ConAddressCity
                ,ConAddressState
