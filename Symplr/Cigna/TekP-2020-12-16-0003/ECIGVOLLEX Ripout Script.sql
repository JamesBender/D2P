...
INSERT INTO [dbo].[AscDefF] (AdfExpression,AdfFieldNumber,AdfForCond,AdfHeaderSystemID,AdfLen,AdfRecType,AdfSetNumber,AdfStartPos,AdfTableName,AdfTargetField,AdfVariableName,AdfVariableType) VALUES ('"drvEmployeeEnrolleeDate"','157','(''UA''=''F'')','ECIGVOLLEXZ0','8','D','10','1667',NULL,'Employee Enrollee Date',NULL,NULL);
...
/*08*/ DECLARE @FILENAME varchar(1000) = 'ECIGVOLLEX_20210510.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Cigna EOI Vol Life Expor-Test','202104289','EMPEXPORT','TEST_XOE','Apr 28 2021  8:54AM','ECIGVOLLEX',NULL,NULL,NULL,'202104289','Apr 28 2021 12:00AM','Dec 30 1899 12:00AM','202104141','510','','','202104141',dbo.fn_GetTimedKey(),NULL,'us3lKiVEN1002',NULL);
...
CREATE TABLE [dbo].[U_ECIGVOLLEX_drvTbl] (
    ...
    [drvEmployeeEnrolleeDate] varchar(8) NULL,
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        ,drvSort = ISNULL(LIFEE_DedCode, 'no') + ' :: ' + ISNULL(LIFES_DedCode, 'no')
        ...
        ,drvEmployeeIssuedAmount = CASE WHEN LIFEE_DedCode IS NOT NULL THEN FORMAT(LIFEE_BenAmt*100, '000000000')
                                        WHEN LIFES_DedCode IS NOT NULL THEN '000000000'
                                    END
        ...
        ,drvEmployeeAppliedAmount = FORMAT(
                                            CASE WHEN LIFEE_DedCode IS NOT NULL THEN 
                                                CASE WHEN LIFEE_EoiAmt <> 0.00 THEN LIFEE_EoiAmt 
                                                ELSE  LIFEE_BenAmt END
                                            WHEN LIFES_DedCode IS NOT NULL THEN '000000000'
                                            END*100, '000000000')
        ,drvSpouseAppliedAmount = FORMAT(
                                            ...
                                            END*100, '000000000')
        ,drvEmployeeEnrolleeDate = CASE WHEN LIFEE_DedCode IS NOT NULL THEN CONVERT(VARCHAR(8), LIFEE_StartDate, 112) ELSE '00000000' END
