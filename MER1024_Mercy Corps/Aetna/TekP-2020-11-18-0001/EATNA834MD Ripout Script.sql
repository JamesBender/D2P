...
/*08*/ DECLARE @FILENAME varchar(1000) = 'EATNA834MD_20210629.txt';
...
INSERT INTO [dbo].[AscExp] (expAscFileName,expAsOfDate,expCOID,expCOIDAllCompanies,expCOIDList,expDateOrPerControl,expDateTimeRangeEnd,expDateTimeRangeStart,expDesc,expEndPerControl,expEngine,expExportCode,expExported,expFormatCode,expGLCodeTypes,expGLCodeTypesAll,expGroupBy,expLastEndPerControl,expLastPayDate,expLastPeriodEndDate,expLastStartPerControl,expNoOfRecords,expSelectByField,expSelectByList,expStartPerControl,expSystemID,expTaxCalcGroupID,expUser,expIEXSystemID) VALUES (RTRIM(@FILEPATH) + LTRIM(RTRIM(@FILENAME)),NULL,'','','',NULL,NULL,NULL,'Full File Only','202105201','EMPEXPORT','FULLFILE','Jun 24 2021 11:20AM','EATNA834MD',NULL,NULL,NULL,'202105201','May 20 2021 12:00AM','Dec 30 1899 12:00AM','202105191','20884','','','202105191',dbo.fn_GetTimedKey(),NULL,'RVAS',NULL);
    ...
    ***************************************************************************************************************/
    ...
    SELECT DISTINCT
        ...
        ,drvHD04_PlanCoverageDesc =    CASE 
                                            WHEN bdmDedCode = 'USHDP' AND bdmBenOption IN  ('EECHLD','EEFAM','EES') AND EepAddressState = 'AR' THEN ''  -- Values supplied by AETNA - waiting on client to update logic to use a custom field for the ones that there are no values
                                            WHEN bdmDedCode = 'USHDP' AND bdmBenOption IN  ('EE') AND EepAddressState = 'AR' THEN ''  -- Values supplied by AETNA - waiting on client to update logic to use a custom field for the ones that there are no values
                                            WHEN bdmDedCode = 'USPPO' AND bdmBenOption IN  ('EE1','EECH1','EES1','EEFAM1') AND EepAddressState = 'AR' THEN ''  -- Values supplied by AETNA - waiting on client to update logic to use a custom field for the ones that there are no values
                                            WHEN bdmDedCode = 'USPPO' AND bdmBenOption IN  ('EE2','EE3','EE4','EE5','EECH2','EECH3','EECH4','EECH5','EES2','EES3','EES4','EES5','EEFAM2','EEFAM3','EEFAM4','EEFAM5') AND EepAddressState = 'AR' THEN ''  -- Values supplied by AETNA - waiting on client to update logic to use a custom field for the ones that there are no values
